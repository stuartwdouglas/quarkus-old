...
public class BootstrapDependencyGraphTransformer implements DependencyGraphTransformer {
    ...
    public BootstrapDependencyGraphTransformer(RepositorySystem system, DependencyGraphTransformer delegate,
            BootstrapDependencyProcessor appDepProcessor, ConflictMarker conflictMarker, ConflictIdSorter conflictIdSorter) {
        this.system = system;
        this.delegate = delegate;
        this.appDepProcessor = appDepProcessor;
        this.conflictMarker = conflictMarker;
        this.conflictIdSorter = conflictIdSorter;
    }
    ...
    @Override
    public BootstrapDependencyGraphTransformer clone() {
        return new BootstrapDependencyGraphTransformer(system, delegate, appDepProcessor, conflictMarker, conflictIdSorter);
    }
    ...
    public DependencyNode transformGraph(DependencyNode node, DependencyGraphTransformationContext context)
        ...
        try {
            ...
            transformed = ctx.isInjectedDeps();
        } finally {
            injectDeps = true;
        }
    ...
    private boolean injectDeps(DependencyNode node, AetherBootstrapDependencyProcessingContext ctx) throws RepositoryException {
        ...
        boolean injectedDeps = false;
        for (DependencyNode child : children) {
                injectedDeps |= injectDeps(child, ctx);
        }
        return injectedDeps;
    }
