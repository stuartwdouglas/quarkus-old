...
public final class BuildChainBuilder {
  ...
  private final Map<BuildStepBuilder, StackTraceElement[]> steps =
      new HashMap<BuildStepBuilder, StackTraceElement[]>();
  ...
  /**
   * Add a build step to the chain. The configuration in the build step builder at the time that the
   * chain is built is the configuration that will apply to the build step in the final chain. Any
   * subsequent changes will be ignored.
   *
   * <p>A given build step is included in the chain when one or more of the following criteria are
   * met:
   *
   * <ul>
   *   <li>It includes a pre-produce step for a item which is produced by one or more build steps
   *       that is included in the chain
   *   <li>It includes a produce step for a item which is consumed by a build step that is included
   *       in the chain or is a final item
   *   <li>It includes a consume step for a item which is produced by a build step that is included
   *       in the chain or is an initial item
   *   <li>It includes a destroy step for a item which is produced by a build step that is included
   *       in the chain or is an initial item
   * </ul>
   *
   * In addition, the declaration of producers and consumers can cause corresponding consumers and
   * producers to be included if they exist.
   *
  ...
  /**
   * Declare an initial item that will be provided to build steps in the chain. Note that if this
   * method is called for a simple item, no build steps will be allowed to produce that item.
   *
  ...
  public BuildChainBuilder loadProviders(ClassLoader classLoader) throws ChainBuildException {
    final ServiceLoader<BuildProvider> serviceLoader =
        ServiceLoader.load(BuildProvider.class, classLoader);
  ...
  /**
   * Declare a final item that will be consumable after the build step chain completes. This may be
   * any item that is produced in the chain.
   *
   * @param type the item type (must not be {@code null})
   * @return this builder
   * @throws IllegalArgumentException if the item type is {@code null}
  ...
  public BuildChain build() throws ChainBuildException {
      ...
      for (Map.Entry<ItemId, Produce> entry : stepProduces.entrySet()) {
        ...
        if (!id.isMulti() && entry.getValue().getConstraint() == Constraint.REAL) {
          ...
          if (initialIds.contains(id)) {
            final ChainBuildException cbe =
                new ChainBuildException(
          ...
          for (Produce produce : list) {
            if (produce.getConstraint() == Constraint.REAL) {
              final Throwable cause =
                  new Throwable("This is the location of the conflicting producer");
              cause.setStackTrace(steps.get(entry.getValue().getStepBuilder()));
              final ChainBuildException cbe =
                  new ChainBuildException("Multiple producers of item " + id, cause);
    ...
    return new BuildChain(
        initialSingleCount, initialMultiCount, startSteps, consumed, this, endSteps.size());
  }

  private static void writeStep(
      final BufferedWriter writer, final HashSet<StepInfo> printed, final StepInfo step)
      throws IOException {
    if (printed.add(step)) {
  ...
  private void cycleCheck(
      Set<BuildStepBuilder> builders,
      Set<BuildStepBuilder> visited,
      Set<BuildStepBuilder> checked,
      final Map<BuildStepBuilder, Set<Produce>> dependencies)
  ...
  private void cycleCheckProduce(
      Set<Produce> produceSet,
      Set<BuildStepBuilder> visited,
      Set<BuildStepBuilder> checked,
      final Map<BuildStepBuilder, Set<Produce>> dependencies)
      throws ChainBuildException {
  ...
  private void cycleCheck(
    ...
    if (!checked.contains(builder)) {
      ...
      try {
        final Set<Produce> dependencySet =
            dependencies.getOrDefault(builder, Collections.emptySet());
  ...
  private void addOne(
      final Map<ItemId, List<Produce>> allProduces,
      final Set<BuildStepBuilder> included,
      final ArrayDeque<BuildStepBuilder> toAdd,
      final ItemId idToAdd,
      Set<Produce> dependencies)
      throws ChainBuildException {
  ...
  private StepInfo buildOne(
      BuildStepBuilder toBuild,
      Set<BuildStepBuilder> included,
      Map<BuildStepBuilder, StepInfo> mapped,
      Map<BuildStepBuilder, Set<BuildStepBuilder>> dependents,
      Map<BuildStepBuilder, Set<Produce>> dependencies,
      final Set<StepInfo> startSteps,
    ...
    final Set<BuildStepBuilder> dependentsOfThis =
        dependents.getOrDefault(toBuild, Collections.emptySet());
    ...
    final Set<Produce> dependenciesOfThis =
        dependencies.getOrDefault(toBuild, Collections.emptySet());
