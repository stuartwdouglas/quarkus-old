...
public final class BuildContext {
  ...
  /**
   * Get the name of this build target. The resultant string is useful for diagnostic messages and
  ...
  /**
   * Produce the given item. If the {@code type} refers to a item which is declared with
   * multiplicity, then this method can be called more than once for the given {@code type},
   * otherwise it must be called no more than once.
   *
   * @param item the item value (must not be {@code null})
   * @throws IllegalArgumentException if the item does not allow multiplicity but this method is
   *     called more than one time, or if the type of item could not be determined
   */
   ...
   * @throws IllegalArgumentException if this deployer was not declared to produce {@code type}, or
   *     if {@code type} is {@code null}, or if the item does not allow multiplicity but this method
   ...
   *     called more than one time, or if the type of item could not be determined
   */
  public <N, T extends NamedBuildItem<N>> void produce(
      Class<T> type, N name, NamedBuildItem<N> item) {
    Assert.checkNotNullParam("type", type);
    Assert.checkNotNullParam("name", name);
    Assert.checkNotNullParam("item", item);
   ...
   * @throws IllegalArgumentException if this deployer was not declared to consume {@code type}, or
  ...
  /**
   * Consume all of the values produced for the named item. If the item type implements {@link
   * Comparable}, it will be sorted by natural order before return. The returned list is a mutable
   * copy.
   *
  ...
  public <T extends MultiBuildItem> List<T> consumeMulti(Class<T> type) {
    ...
    return new ArrayList<>(
        (List<T>) (List) execution.getMultis().getOrDefault(id, Collections.emptyList()));
  }
  ...
  /**
   * Consume all of the values produced for the named item, re-sorting it according to the given
   * comparator. The returned list is a mutable copy.
   *
   * @param type the item element type (must not be {@code null})
   ...
   *     if {@code type} is {@code null}
   */
  public <T extends MultiBuildItem> List<T> consumeMulti(
      Class<T> type, Comparator<? super T> comparator) {
   ...
   *     if {@code type} is {@code null}
   */
  public <N, T extends NamedMultiBuildItem<N>> List<T> consumeMulti(
      Class<T> type, N name, Comparator<? super T> comparator) {
  ...
  /**
   * Determine if a item was produced and is therefore available to be {@linkplain #consume(Class)
   * consumed}.
   *
   * @param type the item type (must not be {@code null})
   * @return {@code true} if the item was produced and is available, {@code false} if it was not or
   *     if this deployer does not consume the named item
   */
  ...
  /**
   * Determine if a item will be consumed in this build. If a item is not consumed, then build steps
   * are not required to produce it.
   *
   * @param type the item type (must not be {@code null})
   * @return {@code true} if the item will be consumed, {@code false} if it will not be or if this
   *     deployer does not produce the named item
   */
  ...
  /**
   * Emit a build warning. This indicates a significant build problem that the user should be made
   * aware of.
   *
   * @param location the location of interest (may be {@code null})
   * @param format the format string (see {@link String#format(String, Object...)})
  ...
  private void doProduce(ItemId id, BuildItem value) {
    ...
    if (id.isMulti()) {
      final List<BuildItem> list =
          execution.getMultis().computeIfAbsent(id, x -> new ArrayList<>());
      synchronized (list) {
  ...
  void depFinished() {
    ...
    log.tracef(
        "Dependency of \"%2$s\" finished; %1$d remaining", remaining, stepInfo.getBuildStep());
    if (remaining == 0) {
          ...
          synchronized (list) {
            list.add(
                new Diagnostic(
                    ...
                    t,
                    null,
                    "Build step %s threw an exception",
                    buildStep));
          }
          execution.setErrorReported();
        }
