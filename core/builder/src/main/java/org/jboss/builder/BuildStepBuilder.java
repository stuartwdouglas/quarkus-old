...
/**
 * A builder for build step instances within a chain. A build step can consume and produce items. It
 * may also register a destructor for items it produces, which will be run (in indeterminate order)
...
public final class BuildStepBuilder {
  ...
  /**
   * Set the build step for this builder. If no build step is specified, then this step will be
   * excluded from the final chain.
   *
  ...
  /**
   * This build step should complete before any build steps which consume the given item {@code
   * type} are initiated. If no such build steps exist, no ordering constraint is enacted.
   *
   ...
   * @return this builder
   */
  public <N> BuildStepBuilder beforeConsume(
      Class<? extends NamedBuildItem<N>> type, N name, ProduceFlag flag) {
    Assert.checkNotNullParam("type", type);
    Assert.checkNotNullParam("name", name);
    Assert.checkNotNullParam("flag", flag);
  ...
  /**
   * This build step should be initiated after any build steps which produce the given item {@code
   * type} are completed. If no such build steps exist, no ordering constraint is enacted.
   *
   * @param type the item type (must not be {@code null})
   * @return this builder
   */
  public BuildStepBuilder afterProduce(Class<? extends BuildItem> type) {
    ...
    addConsumes(
        new ItemId(type, null), Constraint.ORDER_ONLY, ConsumeFlags.of(ConsumeFlag.OPTIONAL));
    return this;
  }
  ...
  public <N> BuildStepBuilder afterProduce(Class<? extends NamedBuildItem<N>> type, N name) {
    ...
    Assert.checkNotNullParam("name", name);
    addConsumes(
        new ItemId(type, name), Constraint.ORDER_ONLY, ConsumeFlags.of(ConsumeFlag.OPTIONAL));
    return this;
  }

  /**
   * Similarly to {@link #beforeConsume(Class)}, establish that this build step must come before the
   * consumer(s) of the given item {@code type}; however, only one {@code producer} may exist for
   * the given item. In addition, the build step may produce an actual value for this item, which
   * will be shared to all consumers during deployment.
   *
   * @param type the item type (must not be {@code null})
   * @return this builder
   */
   ...
   * @return this builder
   */
  public <N> BuildStepBuilder produces(
      Class<? extends NamedBuildItem<N>> type, N name, ProduceFlag flag) {
    Assert.checkNotNullParam("type", type);
    Assert.checkNotNullParam("name", name);
    Assert.checkNotNullParam("flag", flag);
  ...
  public BuildStepBuilder beforeVirtual(Enum<?> symbolic) {
    ...
    addProduces(
        new ItemId(SymbolicBuildItem.class, symbolic), Constraint.ORDER_ONLY, ProduceFlags.NONE);
    return this;
  }
  ...
  public BuildStepBuilder beforeVirtual(Enum<?> symbolic, ProduceFlag flag) {
    ...
    Assert.checkNotNullParam("flag", flag);
    addProduces(
        new ItemId(SymbolicBuildItem.class, symbolic),
        Constraint.ORDER_ONLY,
        ProduceFlags.of(flag));
    return this;
  }

  /**
   * This build step consumes the given produced item. The item must be produced somewhere in the
   * chain. If no such producer exists, the chain will not be constructed; instead, an error will be
   * raised.
   *
   * @param type the item type (must not be {@code null})
   * @return this builder
   ...
   * @return this builder
   */
  public <N> BuildStepBuilder consumes(
      Class<? extends NamedBuildItem<N>> type, N name, ConsumeFlags flags) {
    Assert.checkNotNullParam("type", type);
    Assert.checkNotNullParam("name", name);
  ...
  private void addConsumes(
      final ItemId itemId, final Constraint constraint, final ConsumeFlags flags) {
    Assert.checkNotNullParam("flags", flags);
    consumes.compute(
        itemId,
        (id, c) ->
            c == null
                ? new Consume(this, itemId, constraint, flags)
                : c.combine(constraint, flags));
  }

  private void addProduces(
      final ItemId itemId, final Constraint constraint, final ProduceFlags flags) {
    produces.compute(
        itemId,
        (id, p) ->
            p == null
                ? new Produce(this, itemId, constraint, flags)
                : p.combine(constraint, flags));
  }
