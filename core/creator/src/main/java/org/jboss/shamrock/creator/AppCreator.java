...
public class AppCreator {
  ...
  /**
   * Artifact resolver which should be used to resolve application dependencies. If artifact
   * resolver is not set by the user, the default one will be created based on the user Maven
   * settings.xml file.
   *
  ...
  /**
   * Adds a creation phase to the application creation flow. In the current implementation the
   * phases are processed in the order they are added.
   *
  ...
  /**
   * Work directory used by the phases to store various data. If it's not set by the user, a
   * temporary directory will be created which also be automatically removed after the application
   * have passed through all the creation phases.
   *
  ...
  /**
   * This method allows to push an already available outcome for a certain phase before the build
   ...
   */
  public <O extends AppCreationPhaseOutcome> AppCreator pushOutcome(Class<O> type, O outcome)
    ...
    if (phaseOutcomes.put(type, outcome) != null) {
      ...
      throw new AppCreatorException(
          "Phase outcome of type " + type.getName() + " has already been provided");
    }
    return this;
  }
  ...
  /**
   * Initiates an application creation process for an application artifact coordinates.
   *
  ...
  private void create(
    ...
    try (AppCreationContext ctx = new AppCreationContext(appArtifact, artifactResolver)) {
      ...
      if (phaseOutcomes != null) {
        for (Map.Entry<Class<? extends AppCreationPhaseOutcome>, AppCreationPhaseOutcome> entry :
            phaseOutcomes.entrySet()) {
  ...
  private AppArtifactResolver getDefaultArtifactResolver(Path repoHome) throws AppCreatorException {
    ...
    final DefaultRepositorySystemSession repoSession =
        MavenRepoInitializer.newSession(repoSystem, settings);
    final AppCreatorLocalRepositoryManager appCreatorLocalRepoManager =
        new AppCreatorLocalRepositoryManager(
            repoSystem.newLocalRepositoryManager(
                repoSession, new LocalRepository(repoHome.toString())),
    ...
    final AetherArtifactResolver resolver =
        new AetherArtifactResolver(
            repoSystem, repoSession, MavenRepoInitializer.getRemoteRepos(settings));
