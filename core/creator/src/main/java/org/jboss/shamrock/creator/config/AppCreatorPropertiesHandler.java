...
public class AppCreatorPropertiesHandler implements PropertiesHandler<AppCreator> {
  ...
  private final PropertiesHandler<AugmentPhase> augmentHandler =
      new MappedPropertiesHandler<AugmentPhase>() {
        @Override
        public AugmentPhase newInstance() {
          ...
          .map(
              "wiring-classes",
              (AugmentPhase t, String value) -> t.setWiringClassesDir(Paths.get(value)))
          ...
          .map(
              "uber-jar",
              (AugmentPhase t, String value) -> t.setUberJar(Boolean.parseBoolean(value)));

  private final PropertiesHandler<NativeImagePhase> nativeImageHandler =
      new PropertiesHandler<NativeImagePhase>() {
        @Override
