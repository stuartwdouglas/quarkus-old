...
public class AppCreatorDemo {
  ...
  /**
   * This demo assumes you've built shamrock and its artifacts are in your local repo (otherwise
   * they would have to be available in one of the remote repos to be resolvable)
   *
   * <p>AND also example shamrock-strict-bean-validation-example-1.0.0.Alpha1-SNAPSHOT.jar. This jar
   * is used as an example user app that is augmented and turned into a runnable application
   */
  public static void main(String[] args) throws Exception {
    final Path shamrockRoot = Paths.get("").toAbsolutePath().getParent();
    final Path exampleTarget =
        shamrockRoot.resolve("examples").resolve("bean-validation-strict").resolve("target");

    final Path appJar =
        exampleTarget.resolve("shamrock-strict-bean-validation-example-1.0.0.Alpha1-SNAPSHOT.jar");
  ...
  private static void buildNativeImage(Path userApp, Path outputDir) throws Exception {
    new AppCreator()
        ...
        .addPhase(new AugmentPhase())
        .addPhase(new NativeImagePhase().setOutputDir(outputDir).setDockerBuild(true))
        .create(userApp);
  }

  private static void logLibDiff(final Path exampleTarget, final Path testBuildDir)
      throws IOException {
