...
public class ReportAnalyzer {
  ...
  /**
   * Analyze the contents of the call tree report produced by Substrate when using
   * -H:+PrintAnalysisCallTree, and does a more meaningful analysis of what is causing a type to be
   * retained.
   *
   * <p>In particular for virtual or interface methods that have multiple implementations what is
   * calling this method is not really important, its what caused this particular instance of the
   * class to be created that is important (e.g. if you have an instance of Runnable, you don't care
   * about all the different parts that call runnable, you care about what created this particular
   * instance).
   *
   * <p>If a virtual or interface call is detected with multiple implementations then printing the
   * current call flow is abandoned, and instead the call flow for the constructor of the current
   * object is printed instead.
   */
  public static String analyse(String report, String className, String methodName)
      throws Exception {
    ...
    List<Node> dm =
        byClassMap
            .getOrDefault(className, new ArrayList<>())
            .stream()
            .filter((s) -> s.method.startsWith(methodName + "("))
    ...
    while (!runQueue.isEmpty()) {
      ...
      while (current != null) {
          ...
          if (current.type.equals("is overridden by")) {
            reason =
                "This is an implementation of "
                    ...
                    + " printing path to constructors of "
                    + current.className;
        ...
        if (reason != null) {
          ...
          // note that we discard the string builder if it is part of attemptedClasses, as this
          // basically
