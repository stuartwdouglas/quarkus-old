...
public class AppCreatorDependencySelector implements DependencySelector {
  ...
  private static final Set<String> APP_SCOPES =
      new HashSet<>(Arrays.asList(new String[] {COMPILE, SYSTEM, PROVIDED, RUNTIME}));
  private static final Set<String> TRANSITIVE_SCOPES =
      new HashSet<>(Arrays.asList(new String[] {COMPILE, SYSTEM, RUNTIME}));
  ...
  private AppCreatorDependencySelector(
      AppCreatorDependencySelector parent,
  ...
  public DependencySelector deriveChildSelector(DependencyCollectionContext context) {
    ...
    */
    ...
    if (lastSelected != null
        && !lastSelected.getArtifact().getVersion().equals(dependency.getArtifact().getVersion())) {
    ...
    if (acceptedScopes.size() != TRANSITIVE_SCOPES.size()
        && dependency.getScope().equals(PROVIDED)) {
      return new AppCreatorDependencySelector(
          this,
          TRANSITIVE_SCOPES,
    ...
    return depth < 0 && dependency.getExclusions().isEmpty()
        ? this
        : new AppCreatorDependencySelector(
            this,
            acceptedScopes,
            dependency.getExclusions().isEmpty()
                ? depExclusions
