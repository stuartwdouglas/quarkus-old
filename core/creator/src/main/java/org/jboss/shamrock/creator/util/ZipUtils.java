...
public class ZipUtils {
  ...
  public static void zip(Path src, Path zipFile) throws IOException {
    try (FileSystem zipfs =
        newFileSystem(
            toZipUri(zipFile), Files.exists(zipFile) ? Collections.emptyMap() : CREATE_ENV)) {
      if (Files.isDirectory(src)) {
        ...
        Files.copy(
            src, zipfs.getPath(src.getFileName().toString()), StandardCopyOption.REPLACE_EXISTING);
      }
    }
  }
    ...
    Files.walkFileTree(
        ...
        new SimpleFileVisitor<Path>() {
          ...
          public FileVisitResult visitFile(Path file, BasicFileAttributes attrs)
            ...
            Files.copy(
                file,
                zipfs.getPath(srcRoot.relativize(file).toString()),
                StandardCopyOption.REPLACE_EXISTING);
            return FileVisitResult.CONTINUE;
          }
        });
  ...
  /**
   * This call is not thread safe, a single of FileSystem can be created for the profided uri until
   * it is closed.
   *
