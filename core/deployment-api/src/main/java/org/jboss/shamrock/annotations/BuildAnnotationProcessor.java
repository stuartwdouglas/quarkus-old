...
public class BuildAnnotationProcessor extends AbstractProcessor {
  ...
  private static final String STATIC_RECORDER =
      "org.jboss.shamrock.deployment.builditem.StaticBytecodeRecorderBuildItem";
  private static final String MAIN_RECORDER =
      "org.jboss.shamrock.deployment.builditem.MainBytecodeRecorderBuildItem";
  ...
  private static final String CONFIG_PROPERTY_ANNOTATION =
  ...
  public void doProcess(Set<? extends TypeElement> annotations, RoundEnvironment roundEnv) {
    ...
    // process each class individually, we only create a single instance of each even if it has
    // multiple steps
    for (TypeElement processor : processorClasses) {
      ...
      for (VariableElement element : fieldsIn(processor.getEnclosedElements())) {
        try {
          ...
          throw new RuntimeException(
              "Exception processing field " + element + " in type " + processor, e);
        }
      }
      ...
      String processorClassName =
          processingEnv.getElementUtils().getBinaryName(processor).toString();
      ...
      try (ClassCreator creator =
          new ClassCreator(
              new ProcessorClassOutput(processor),
              buildProviderName,
              null,
              Object.class.getName(),
              BuildProvider.class.getName())) {
        MethodCreator mc =
            creator.getMethodCreator("installInto", void.class, BuildChainBuilder.class);

        ResultHandle theInstance = mc.newInstance(ofConstructor(processorClassName));

        for (VariableElement i : configuredFields) {
          injectConfigField(
              mc, theInstance, i, processorClassName, mc.load(""), configProperties, "");
        }
        ...
        for (ExecutableElement method : methodMap.get(processor)) {
          try {
            if (method.getModifiers().contains(Modifier.PRIVATE)) {
              throw new RuntimeException(
                  "@BuildStep methods cannot be private: " + processorClassName + ":" + method);
            }
            ...
            for (VariableElement i : method.getParameters()) {
              ...
              String simpleType =
                  processingEnv
                      .getElementUtils()
                      .getBinaryName(((TypeElement) type.asElement()))
                      .toString();
            ...
            if (recordAnnotation == null && templatePresent) {
              throw new RuntimeException(
                  "Cannot inject @Template classes into methods that are not annotated @Record: "
                      + method);
            } else if (recordAnnotation != null && !templatePresent) {
              throw new RuntimeException(
                  "@Record method does not inject any template classes " + method);
            ...
            try (ClassCreator buildStepCreator =
                new ClassCreator(
                    new ProcessorClassOutput(processor),
                    buildStepName,
                    null,
                    Object.class.getName(),
                    org.jboss.builder.BuildStep.class.getName())) {

              // the constructor, just sets the instance field
              MethodCreator ctor =
                  buildStepCreator.getMethodCreator(
                      ofConstructor(buildStepName, processorClassName));
              ctor.invokeSpecialMethod(ofConstructor(Object.class), ctor.getThis());
              FieldCreator ifield =
                  buildStepCreator.getFieldCreator("instance", processorClassName);
              ctor.writeInstanceField(
                  ifield.getFieldDescriptor(), ctor.getThis(), ctor.getMethodParam(0));
              ctor.returnValue(null);

              // toString
              MethodCreator toString = buildStepCreator.getMethodCreator("toString", String.class);
              toString.returnValue(
                  toString.load(processorClassName + "." + method.getSimpleName()));

              MethodCreator buildStepMc =
                  buildStepCreator.getMethodCreator("execute", void.class, BuildContext.class);
              ResultHandle p =
                  buildStepMc.readInstanceField(ifield.getFieldDescriptor(), buildStepMc.getThis());
              ...
              if (templatePresent) {
                bytecodeRecorder =
                    buildStepMc.newInstance(
                        ofConstructor(
                            "org.jboss.shamrock.deployment.recording.BytecodeRecorderImpl",
                            boolean.class,
                            String.class,
                            String.class),
                        buildStepMc.load(recordAnnotation.value() == ExecutionTime.STATIC_INIT),
                        buildStepMc.load(method.getEnclosingElement().getSimpleName().toString()),
                        buildStepMc.load(method.getSimpleName().toString()));
              }
              ...
              ResultHandle handle =
                  buildStepMc.invokeVirtualMethod(
                      ofMethod(
                          processorClassName,
                          method.getSimpleName().toString(),
                          returnInfo.rawReturnType,
                          methodParamTypes.toArray(new String[0])),
                      p,
              ...
              if (returnInfo.producedReturnType != null) {
                ...
                if (returnInfo.list) {
                  ifstm
                      .falseBranch()
                      .invokeVirtualMethod(
                          ofMethod(BuildContext.class, "produce", void.class, List.class),
                          buildStepMc.getMethodParam(0),
                          handle);
                } else {
                  ifstm
                      .falseBranch()
                      .invokeVirtualMethod(
                          ofMethod(BuildContext.class, "produce", void.class, BuildItem.class),
                          buildStepMc.getMethodParam(0),
                          handle);
                }
              }
              ...
              catchBlockCreator.throwException(
                  RuntimeException.class,
                  "Failed to process build step",
            ...
            throw new RuntimeException(
                "Failed to process " + processorClassName + "." + method.getSimpleName(), e);
          }
        }
    ...
    if (!serviceNames.isEmpty()) {
      // we read them first, as if an IDE has processed this we may not have seen the full set of
      // names
      try {
        ...
        FileObject res =
            processingEnv
                ...
                .createResource(
                    ...
                    "",
                    relativeName,
                    processorElements.toArray(new Element[0]));
        ...
        FileObject descriptions =
            processingEnv
                .getFiler()
                .createResource(
                    StandardLocation.CLASS_OUTPUT, "", "META-INF/shamrock-descriptions.properties");
  ...
  private void injectConfigField(
      BytecodeCreator bytecode,
      ResultHandle obj,
      VariableElement field,
      String className,
      ResultHandle keyPrefix,
      Properties configProperties,
      String currentKeyDesc) {
    ConfigInjectionInfo val = readConfigAnnotation(field);
    ResultHandle value =
        createConfigValue(bytecode, val, keyPrefix, configProperties, currentKeyDesc);
    String jvmType;
    switch (val.type) {
    ...
    if (val.optional) {
      ResultHandle instance =
          bytecode.invokeStaticMethod(
              ofMethod(Optional.class, "ofNullable", Optional.class, Object.class), value);
      bytecode.writeInstanceField(
          FieldDescriptor.of(className, field.getSimpleName().toString(), Optional.class),
          obj,
          instance);
    } else {
      bytecode.writeInstanceField(
          FieldDescriptor.of(className, field.getSimpleName().toString(), jvmType), obj, value);
    }
  }

  private ResultHandle createConfigValue(
      BytecodeCreator bc,
      ConfigInjectionInfo val,
      ResultHandle keyPrefix,
      Properties configProperties,
      String currentKeyDescPrefix) {
    String currentKeyDesc =
        currentKeyDescPrefix.isEmpty() ? val.name : (currentKeyDescPrefix + "." + val.name);
    ResultHandle sbr = bc.newInstance(ofConstructor(StringBuilder.class));
    bc.invokeVirtualMethod(
        ofMethod(StringBuilder.class, "append", StringBuilder.class, String.class), sbr, keyPrefix);
    bc.invokeVirtualMethod(
        ofMethod(StringBuilder.class, "append", StringBuilder.class, String.class),
        sbr,
        bc.load(val.name));
    ResultHandle keyName =
        bc.invokeVirtualMethod(
            ofMethod(Object.class, "toString", String.class), sbr); // the value name
    ...
    switch (val.type) {
      case STRING:
        return bc.invokeStaticMethod(
            ...
            keyName,
            defaultVal,
            val.optional ? bc.load(true) : bc.load(false));
      case PRIMITIVE_BOOLEAN:
        return bc.invokeStaticMethod(
            ofMethod(SHAMROCK_CONFIG, "getBoolean", boolean.class, String.class, String.class),
            keyName,
            defaultVal);
      case PRIMITIVE_INT:
        return bc.invokeStaticMethod(
            ofMethod(SHAMROCK_CONFIG, "getInt", int.class, String.class, String.class),
            keyName,
            defaultVal);
      case INTEGER:
        return bc.invokeStaticMethod(
      ...
      case CUSTOM_TYPE:
        {
          ...
          if (element == null) {
            throw new RuntimeException(
                "Could not obtain class information for " + val.customTypeName);
          }
          ...
          bc.invokeVirtualMethod(
              ...
              sbr,
              bc.load("."));
          ResultHandle keyPrefixName =
              bc.invokeVirtualMethod(
                  ofMethod(Object.class, "toString", String.class), sbr); // the name + a .
      ...
      case MAP:
        ...
        ResultHandle names =
            bc.invokeStaticMethod(
                ofMethod(SHAMROCK_CONFIG, "getNames", Set.class, String.class), keyName);
        FunctionCreator func = bc.createFunction(Consumer.class);
        bc.invokeInterfaceMethod(
            ofMethod(Set.class, "forEach", void.class, Consumer.class), names, func.getInstance());
          ...
          if (isAnnotationPresent(e, CONFIG_PROPERTY_ANNOTATION)) {
            ...
            func.getBytecode()
                .invokeVirtualMethod(
                    ofMethod(StringBuilder.class, "append", StringBuilder.class, String.class),
                    sb,
                    func.getBytecode().getMethodParam(0));
            func.getBytecode()
                .invokeVirtualMethod(
                    ofMethod(StringBuilder.class, "append", StringBuilder.class, String.class),
                    sb,
                    func.getBytecode().load("."));
            ResultHandle handle =
                func.getBytecode()
                    .invokeVirtualMethod(ofMethod(Object.class, "toString", String.class), sb);
            injectConfigField(
                func.getBytecode(),
                mi,
                (VariableElement) e,
        ...
        func.getBytecode()
            .invokeVirtualMethod(
                ofMethod(HashMap.class, "put", Object.class, Object.class, Object.class),
                instance,
                func.getBytecode().getMethodParam(0),
                mi);
  ...
  private ConfigInjectionInfo readConfigAnnotation(VariableElement field) {
    for (AnnotationMirror i : field.getAnnotationMirrors()) {
      if (((TypeElement) i.getAnnotationType().asElement())
          .getQualifiedName()
          .toString()
          .equals(CONFIG_PROPERTY_ANNOTATION)) {
        ConfigInjectionInfo ret = new ConfigInjectionInfo();
        for (Map.Entry<? extends ExecutableElement, ? extends AnnotationValue> elem :
            i.getElementValues().entrySet()) {
        ...
        if (ret.name == null) {
          String msg =
              "@ConfigProperty with no name specified "
        ...
        if (field.asType() instanceof PrimitiveType) {
          ...
          switch (type.getKind()) {
            ...
            default:
              throw new RuntimeException(
                  "Unable to inject config into primitive type of "
                      + type
          ...
          if (simpleType.equals(Map.class.getName())) {
            ...
            if (type.getTypeArguments().size() != 2) {
              throw new RuntimeException(
                  "Cannot use @ConfigProperty on a Map that does not include a generic type "
                      + field);
            }
            ...
            ret.customTypeName =
                processingEnv
                    .getElementUtils()
                    .getBinaryName(((TypeElement) ((DeclaredType) typeMirror).asElement()))
                    .toString();
          } else if (simpleType.equals(Optional.class.getName())) {
            if (type.getTypeArguments().size() != 1) {
              throw new RuntimeException(
                  "Cannot use @ConfigProperty on an Optional that does not include a generic type "
                      + field);
            }
            ret.optional = true;
            ...
            if (typeName.equals(String.class.getName())) {
              ...
              if (!isAnnotationPresent(
                  processingEnv.getTypeUtils().asElement(typeMirror), CONFIG_GROUP_ANNOTATION)) {
                throw new RuntimeException(
                    "Cannot inject a configured instance of "
                        + typeName
                        + " as it is not annotated with "
            ...
            ret.type = ConfigType.CUSTOM_TYPE;
            if (!isAnnotationPresent(
                processingEnv.getTypeUtils().asElement(type), CONFIG_GROUP_ANNOTATION)) {
              throw new RuntimeException(
                  "Cannot inject a configured instance of "
                      + simpleType
                      + " as it is not annotated with "
        ...
        if (ret.optional && ret.defaultValue != null) {
          throw new RuntimeException(
              field
                  + " is optional but has a default value. Default values must not be set for optional elements");
        }
        ...
        if (ret.type != ConfigType.MAP && ret.type != ConfigType.CUSTOM_TYPE) {
          ...
          if (rawJavadoc == null) {
            ...
            if (rawJavadoc == null) {
              throw new RuntimeException(
                  "Field must include a javadoc description "
                      + field.getEnclosingElement()
                      + "."
                      + field);
            }
  ...
  private String tryLoadJavadocFromFile(VariableElement field) {
    ...
    try {
      FileObject file =
          processingEnv
              ...
              .getResource(
                  ...
                  "",
                  ownerClass.getQualifiedName().toString().replace(".", "/") + ".confjavadoc");
      ...
      processingEnv
          .getMessager()
          .printMessage(Diagnostic.Kind.WARNING, "Unable to load pre-saved javadoc", field);
    }
  ...
  private MethodReturnInfo processMethodReturnType(ExecutableElement method) {
    ...
    if (method.getReturnType().getKind() != TypeKind.VOID) {
      ...
      String returnType =
          processingEnv
              .getElementUtils()
              .getBinaryName(((TypeElement) returnTypeElement.asElement()))
              .toString();
      ...
      if (returnType.equals(List.class.getName())) {
        ...
        if (returnTypeElement.getTypeArguments().size() != 1) {
          throw new RuntimeException(
              "Cannot use @BuildResource on a list that does not include a generic type");
        }
  ...
  private String registerBuildStep(
      List<InjectedBuildResource> fieldList,
      String processorClassName,
      MethodCreator mc,
      String producedReturnType,
      List<InjectedBuildResource> methodInjection,
      boolean templatePresent,
      Record recordAnnotation,
      ResultHandle theInstance) {
    final String buildStepName =
        processorClassName + "BuildStep" + classNameCounter.incrementAndGet();

    ResultHandle step =
        mc.newInstance(ofConstructor(buildStepName, processorClassName), theInstance);
    ResultHandle builder =
        mc.invokeVirtualMethod(
            MethodDescriptor.ofMethod(
                BuildChainBuilder.class,
                "addBuildStep",
                BuildStepBuilder.class,
                org.jboss.builder.BuildStep.class),
            mc.getMethodParam(0),
            step);
    ...
    for (InjectedBuildResource field : fieldList) {
      if (field.consumedTypeName != null) {
        if (field.injectionType == InjectionType.OPTIONAL) {
          ResultHandle consumeFlags =
              mc.invokeStaticMethod(
                  ofMethod(ConsumeFlags.class, "of", ConsumeFlags.class, ConsumeFlag.class),
          ...
          mc.invokeVirtualMethod(
              ofMethod(
                  ...
                  ConsumeFlags.class),
              builder,
              mc.loadClass(field.consumedTypeName),
              consumeFlags);
        } else {
          mc.invokeVirtualMethod(
              ofMethod(BuildStepBuilder.class, "consumes", BuildStepBuilder.class, Class.class),
              builder,
              mc.loadClass(field.consumedTypeName));
        }
      }
      if (field.producedTypeName != null) {
        mc.invokeVirtualMethod(
            ofMethod(BuildStepBuilder.class, "produces", BuildStepBuilder.class, Class.class),
            builder,
            mc.loadClass(field.producedTypeName));
      }
    }
    ...
    if (templatePresent) {
      ...
      if (recordAnnotation.optional()) {
        mc.invokeVirtualMethod(
            ...
            builder,
            type,
            mc.readStaticField(FieldDescriptor.of(ProduceFlag.class, "WEAK", ProduceFlag.class)));
      } else {
        mc.invokeVirtualMethod(
            ofMethod(BuildStepBuilder.class, "produces", BuildStepBuilder.class, Class.class),
            builder,
    ...
    for (InjectedBuildResource injection : methodInjection) {
      if (injection.injectionType != InjectionType.TEMPLATE
          && injection.injectionType != InjectionType.RECORDER_CONTEXT
          && injection.injectionType != InjectionType.EXECUTOR) {
          ...
          if (injection.injectionType == InjectionType.OPTIONAL) {
            ...
            mc.invokeVirtualMethod(
                ofMethod(
                    ...
                    ConsumeFlags.class),
                builder,
                mc.loadClass(injection.consumedTypeName),
                consumeFlags);
          } else {
            mc.invokeVirtualMethod(
                ofMethod(BuildStepBuilder.class, "consumes", BuildStepBuilder.class, Class.class),
        ...
        if (injection.producedTypeName != null) {
          mc.invokeVirtualMethod(
              ofMethod(BuildStepBuilder.class, "produces", BuildStepBuilder.class, Class.class),
              builder,
              mc.loadClass(injection.producedTypeName));
        }
      }
    }
    ...
    if (producedReturnType != null) {
      mc.invokeVirtualMethod(
          ofMethod(BuildStepBuilder.class, "produces", BuildStepBuilder.class, Class.class),
          builder,
          mc.loadClass(producedReturnType));
    }

    // install it
    mc.invokeVirtualMethod(
        ofMethod(BuildStepBuilder.class, "build", BuildChainBuilder.class), builder);
    return buildStepName;
  }

  private ResultHandle generateMethodInjection(
      MethodCreator buildStepMc, ResultHandle bytecodeRecorder, InjectedBuildResource i) {
    ResultHandle val;
      ...
      val =
          buildStepMc.invokeVirtualMethod(
              ofMethod(
                  ...
                  Class.class),
              bytecodeRecorder,
              buildStepMc.loadClass(i.consumedTypeName));
    } else if (i.injectionType == InjectionType.EXECUTOR) {
      val =
          buildStepMc.invokeVirtualMethod(
              ofMethod(BuildContext.class, "getExecutor", Executor.class),
              buildStepMc.getMethodParam(0));
    } else if (i.injectionType == InjectionType.SIMPLE) {
      val =
          buildStepMc.invokeVirtualMethod(
              ofMethod(BuildContext.class, "consume", SimpleBuildItem.class, Class.class),
              buildStepMc.getMethodParam(0),
              buildStepMc.loadClass(i.consumedTypeName));
    } else if (i.injectionType == InjectionType.LIST) {
      val =
          buildStepMc.invokeVirtualMethod(
              ofMethod(BuildContext.class, "consumeMulti", List.class, Class.class),
              buildStepMc.getMethodParam(0),
              buildStepMc.loadClass(i.consumedTypeName));
    } else if (i.injectionType == InjectionType.OPTIONAL) {
      ResultHandle tempVal =
          buildStepMc.invokeVirtualMethod(
              ofMethod(BuildContext.class, "consume", SimpleBuildItem.class, Class.class),
              buildStepMc.getMethodParam(0),
              buildStepMc.loadClass(i.consumedTypeName));
      val =
          buildStepMc.invokeStaticMethod(
              ofMethod(Optional.class, "ofNullable", Optional.class, Object.class), tempVal);
    } else {
      val =
          buildStepMc.newInstance(
              ofConstructor(BUILD_PRODUCER, Class.class, BuildContext.class),
              buildStepMc.loadClass(i.producedTypeName),
              buildStepMc.getMethodParam(0));
    }
    return val;
  }

  private void generateFieldInjection(
      String processorClassName,
      MethodCreator buildStepMc,
      ResultHandle p,
      InjectedBuildResource field) {
    if (field.injectionType == InjectionType.SIMPLE) {
      ResultHandle val =
          buildStepMc.invokeVirtualMethod(
              ofMethod(BuildContext.class, "consume", SimpleBuildItem.class, Class.class),
              buildStepMc.getMethodParam(0),
              buildStepMc.loadClass(field.consumedTypeName));
      buildStepMc.writeInstanceField(
          FieldDescriptor.of(
              processorClassName, field.element.getSimpleName().toString(), field.consumedTypeName),
          p,
          val);
      ...
      buildStepMc.writeInstanceField(
          FieldDescriptor.of(
              processorClassName, field.element.getSimpleName().toString(), List.class),
          p,
          val);
      ...
      ResultHandle optional =
          buildStepMc.invokeStaticMethod(
              ofMethod(Optional.class, "ofNullable", Optional.class, Object.class), val);
      buildStepMc.writeInstanceField(
          FieldDescriptor.of(
              processorClassName, field.element.getSimpleName().toString(), Optional.class),
          p,
          optional);
      ...
      buildStepMc.writeInstanceField(
          FieldDescriptor.of(
              processorClassName, field.element.getSimpleName().toString(), Executor.class),
          p,
          val);
    } else if (field.injectionType == InjectionType.TEMPLATE
        || field.injectionType == InjectionType.RECORDER_CONTEXT) {
      throw new RuntimeException(
          "Cannot inject @Template class into a field, only method parameter injection is supported for templates. Field: "
              + field.element);
    } else {
      ResultHandle val =
          buildStepMc.newInstance(
              ofConstructor(BUILD_PRODUCER, Class.class, BuildContext.class),
              buildStepMc.loadClass(field.producedTypeName),
              buildStepMc.getMethodParam(0));
      buildStepMc.writeInstanceField(
          FieldDescriptor.of(
              processorClassName, field.element.getSimpleName().toString(), BuildProducer.class),
          p,
          val);
    }
  }
    ...
    if (capabilities.length > 0) {
      for (String i : capabilities) {
        step =
            mc.newInstance(
                ofConstructor(
                    "org.jboss.shamrock.deployment.steps.CapabilityBuildStep", String.class),
                mc.load(i));
        builder =
            mc.invokeVirtualMethod(
                MethodDescriptor.ofMethod(
        ...
        mc.invokeVirtualMethod(
            ofMethod(BuildStepBuilder.class, "produces", BuildStepBuilder.class, Class.class),
            builder,
            mc.loadClass("org.jboss.shamrock.deployment.builditem.CapabilityBuildItem"));
        mc.invokeVirtualMethod(
            ofMethod(BuildStepBuilder.class, "build", BuildChainBuilder.class), builder);
      }
    }
    ...
    if (markers.length > 0) {
      for (String i : markers) {
        step =
            mc.newInstance(
                ofConstructor(
                    "org.jboss.shamrock.deployment.steps.ApplicationArchiveMarkerBuildStep",
                    String.class),
                mc.load(i));
        builder =
            mc.invokeVirtualMethod(
        ...
        mc.invokeVirtualMethod(
            ...
            builder,
            mc.loadClass(
                "org.jboss.shamrock.deployment.builditem.AdditionalApplicationArchiveMarkerBuildItem"));
        mc.invokeVirtualMethod(
            ofMethod(BuildStepBuilder.class, "build", BuildChainBuilder.class), builder);
      }
    }
  ...
  private InjectedBuildResource createInjectionResource(Element element, DeclaredType elementType) {
    ...
    String simpleType =
        processingEnv.getElementUtils().getBinaryName(((TypeElement) type.asElement())).toString();
    InjectionType ft;
    ...
    if (simpleType.equals(List.class.getName())) {
      ...
      if (type.getTypeArguments().size() != 1) {
        throw new RuntimeException(
            "Cannot use @BuildResource on an optional that does not include a generic type");
      }
      TypeMirror typeMirror = type.getTypeArguments().get(0);
      ...
      if (type.getTypeArguments().size() != 1) {
        throw new RuntimeException(
            "Cannot use @BuildResource on a BuildProducer that does not include a generic type");
      }
      TypeMirror typeMirror = type.getTypeArguments().get(0);
  ...
  private void verifyType(TypeMirror type, Class expected) {
    if (!processingEnv
        .getTypeUtils()
        .isSubtype(
            type, processingEnv.getElementUtils().getTypeElement(expected.getName()).asType())) {
  ...
  private class ProcessorClassOutput implements ClassOutput {
    ...
    public void write(String name, byte[] data) {
      try (OutputStream out =
          processingEnv
              .getFiler()
              .createClassFile(name.replace("/", "."), processor)
              .openOutputStream()) {
        out.write(data);
      } catch (IOException e) {
        throw new RuntimeException(e);
