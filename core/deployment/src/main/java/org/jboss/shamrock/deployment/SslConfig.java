...
public class SslConfig {
  ...
  SystemPropertyBuildItem setupNativeSsl() {
    ...
    if (enableSsl) {
      // I assume we only fail if we actually enable it, but perhaps there's a no-native called that
      // we can't
      // see here?
      ...
      return new SystemPropertyBuildItem(
          "java.library.path",
          graalVmHome + File.separator + "jre" + File.separator + "lib" + File.separator + "amd64");
    }
    return null;
  }
}
