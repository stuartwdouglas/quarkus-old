 ...
 *
 * <p>This assumes that all artifacts to be resolved have a META-INF/MANIFEST.MF file, and are layed
 * out in the maven repository structure on the file system.
 */
        ...
        if (path.startsWith("file:")) {
          pathList.add(
              new StoredUrl(
                  Paths.get(
                      new URI(
                          path.substring(0, path.length() - META_INF_MANIFEST_MF.length() - 2)))));
        }
      }
    ...
    for (StoredUrl url : pathList) {
      ...
      if (matcher.matches()) {
        ...
        for (int i = 0; i < groupParts.length; ++i) {
          String up =
              url.path.getName(url.path.getNameCount() - groupParts.length - 3 + i).toString();
        ...
        if (matches) {
          try {
            return new ResolvedArtifact(
                groupId, artifactId, matcher.group(1), classifier, url.path);
          } catch (Exception e) {
            throw new RuntimeException(e);
          }
        }
