...
public interface RecorderContext {
  ...
  /**
   * Registers a way to construct an object via a non-default constructor. Each object may only have
   * at most one non-default constructor registered
   *
  ...
  /**
   * Registers a substitution to allow objects that are not serialisable to bytecode to be
   * substituted for an object that is.
   *
   ...
   */
  <F, T> void registerSubstitution(
      Class<F> from, Class<T> to, Class<? extends ObjectSubstitution<F, T>> substitution);

  /**
   * Creates a Class instance that can be passed to a recording proxy as a substitute for a class
   * that is not loadable at processing time. At runtime the actual class will be passed into the
   * invoked method.
   *
   ...
   *
   * <p>This object can be passed into templates, but must not be used directly at deployment time
   *
