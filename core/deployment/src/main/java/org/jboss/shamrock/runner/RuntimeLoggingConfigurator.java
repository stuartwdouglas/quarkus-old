...
/**
 * Embedded configurator that can be used to configure logging in a similar manner to the auto
...
public class RuntimeLoggingConfigurator implements EmbeddedConfigurator {
  ...
  public Level getLevelOf(final String loggerName) {
    ...
    Optional<String> level =
        config.getOptionalValue("shamrock.log.category." + loggerName + ".level", String.class);
    if (level.isPresent()) {
  ...
  public Handler[] getHandlersOf(final String loggerName) {
    ...
    String format =
        config
            .getOptionalValue("shamrock.log.console.format", String.class)
            .orElse("%d{yyyy-MM-dd HH:mm:ss,SSS} %h %N[%i] %-5p [%c{1.}] (%t) %s%e%n");
    boolean color =
        config.getOptionalValue("shamrock.log.console.format", Boolean.class).orElse(true);

    if (color && System.console() != null) {
      return loggerName.isEmpty()
          ? new Handler[] {new ConsoleHandler(new ColorPatternFormatter(format))}
          : NO_HANDLERS;
    } else {
      return loggerName.isEmpty()
          ? new Handler[] {new ConsoleHandler(new PatternFormatter(format))}
          : NO_HANDLERS;
    }
  }
