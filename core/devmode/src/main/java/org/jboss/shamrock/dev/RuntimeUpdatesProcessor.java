...
public class RuntimeUpdatesProcessor {
  ...
  private static final Logger log =
      Logger.getLogger(RuntimeUpdatesProcessor.class.getPackage().getName());
  ...
  public RuntimeUpdatesProcessor(
      Path classesDir, Path sourcesDir, Path resourcesDir, ClassLoaderCompiler compiler) {
          ...
          classesStream
              ...
              .collect(
                  Collectors.toConcurrentMap(
                      p -> pathToClassName(p), p -> CopyUtils.readFileContentNoIOExceptions(p)));
    }
    ...
    public void handle(Map<String, byte[]> changed) {
      ...
      for (Map.Entry<String, byte[]> e : changed.entrySet()) {
        ...
        try {
          cd =
              new ClassDefinition(
                  Class.forName(e.getKey(), false, DevModeMain.getCurrentAppClassLoader()),
                  e.getValue());
