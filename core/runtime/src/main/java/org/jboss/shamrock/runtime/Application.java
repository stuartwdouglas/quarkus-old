...
/**
 * The application base class, which is extended and implemented by a generated class which
 * implements the application setup logic. The base class does some basic error checking.
 */
...
public abstract class Application {
  ...
  /**
   * Start the application. If another thread is also trying to start the application, this method
   * waits for that thread to finish starting. Returns immediately if the application is started
   * already. If the application fails to start, an exception is thrown.
   *
   * @param args the command-line arguments
   * @implNote The command line args are not yet used, but at some point we'll want a facility for
   *     overriding config and/or letting the user hook into it.
   */
  ...
  /**
   * Stop the application. If another thread is also trying to stop the application, this method
   * waits for that thread to finish. Returns immediately if the application is already stopped. If
   * an exception is thrown during stop, that exception is propagated.
   */
  public final void stop() {
    ...
    try {
      ...
      try {
        ...
        Logger.getLogger("org.jboss.shamrock")
            .infof(
                "Shamrock stopped in %d.%03dms",
                Long.valueOf(time / 1_000_000), Long.valueOf(time % 1_000_000 / 1_000));
        stateCond.signalAll();
      } finally {
        stateLock.unlock();
      }
  ...
  private static IllegalStateException interruptedOnAwaitStart() {
    return new IllegalStateException(
        "Interrupted while waiting for another thread to start the application");
  }

  private static IllegalStateException interruptedOnAwaitStop() {
    return new IllegalStateException(
        "Interrupted while waiting for another thread to stop the application");
  }
