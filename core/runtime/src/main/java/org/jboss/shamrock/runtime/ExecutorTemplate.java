...
public class ExecutorTemplate {
  ...
  public Executor setupRunTime(
      ShutdownContext shutdownContext,
      int defaultCoreSize,
      int defaultMaxSize,
      int defaultQueueSize,
      float defaultGrowthResistance,
      int defaultKeepAliveMillis) {
    final JBossThreadFactory threadFactory =
        new JBossThreadFactory(
            new ThreadGroup("executor"),
            Boolean.TRUE,
            null,
            "executor-thread-%t",
            JBossExecutors.loggingExceptionHandler("org.jboss.executor.uncaught"),
            null);
    final EnhancedQueueExecutor.Builder builder =
    ...
    builder.setCorePoolSize(
        getIntConfigVal(CORE_POOL_SIZE, defaultCoreSize == -1 ? 4 * cpus : defaultCoreSize));
    builder.setMaximumPoolSize(
        getIntConfigVal(MAX_POOL_SIZE, defaultMaxSize == -1 ? 10 * cpus : defaultMaxSize));
    ...
    builder.setKeepAliveTime(
        getIntConfigVal("executor.keep-alive-millis", defaultKeepAliveMillis),
