...
public class ConfigBuildStep {
  ...
  static class ConfigBeanDeploymentValidator implements BeanDeploymentValidator {
    ...
    private static final DotName CONFIG_PROPERTY_NAME =
        DotName.createSimple(ConfigProperty.class.getName());
          ...
          try {
            ...
            if (!config.getOptionalValue(key, type).isPresent()) {
              ...
              if (defaultValue == null
                  || ConfigProperty.UNCONFIGURED_VALUE.equals(defaultValue.asString())) {
                validationContext.addDeploymentProblem(
                    new IllegalStateException("No config value exists for: " + key));
              }
            }
          } catch (ClassNotFoundException e) {
            throw new IllegalStateException(
                "Unable to verify config injection point: " + injectionPoint, e);
          }
        }
      }
    }
