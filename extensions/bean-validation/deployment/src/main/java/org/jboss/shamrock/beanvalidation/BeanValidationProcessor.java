...
class BeanValidationProcessor {
  ...
  private static final DotName VALIDATE_ON_EXECUTION =
      DotName.createSimple(ValidateOnExecution.class.getName());
    ...
    if (isResteasyInClasspath()) {
      ...
      additionalBeans.produce(
          new AdditionalBeanBuildItem(
              "org.jboss.shamrock.beanvalidation.runtime.jaxrs.JaxrsEndPointValidationInterceptor"));
    }
  }
  ...
  public void build(
    ...
    for (AnnotationInstance constraint :
        indexView.getAnnotations(DotName.createSimple(Constraint.class.getName()))) {
    ...
    for (DotName consideredAnnotation : consideredAnnotations) {
      Collection<AnnotationInstance> annotationInstances =
          indexView.getAnnotations(consideredAnnotation);
      ...
      for (AnnotationInstance annotation : annotationInstances) {
        if (annotation.target().kind() == AnnotationTarget.Kind.FIELD) {
          ...
          contributeClassMarkedForCascadingValidation(
              classNamesToBeValidated,
              indexView,
          ...
          // a getter does not have parameters so it's a pure method: no need for reflection in this
          // case
          contributeClassMarkedForCascadingValidation(
              classNamesToBeValidated,
              indexView,
    ...
    // Add the annotations transformer to add @MethodValidated annotations on the methods requiring
    // validation
    annotationsTransformers.produce(
  ...
  private static void contributeBuiltinConstraints(Set<DotName> constraintCollector) {
    Set<Class<? extends Annotation>> builtinConstraints =
        new ConstraintHelper().getBuiltinConstraints();
  ...
  private static void contributeClass(
      Set<DotName> classNamesCollector, IndexView indexView, DotName className) {
  ...
  private static void contributeClassMarkedForCascadingValidation(
      Set<DotName> classNamesCollector,
      IndexView indexView,
      DotName consideredAnnotation,
      Type type) {
