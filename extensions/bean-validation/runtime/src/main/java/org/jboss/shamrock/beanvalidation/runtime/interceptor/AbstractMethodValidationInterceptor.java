package org.jboss.shamrock.beanvalidation.runtime.interceptor;
...
/**
 * NOTE: this is a copy of the interceptor present in hibernate-validator-cdi. For now, I prefer not
 * depending on this artifact but this might change in the future.
 *
 * <p>An interceptor which performs a validation of the Bean Validation constraints specified at the
 * parameters and/or return values of intercepted methods using the method validation functionality
...
public abstract class AbstractMethodValidationInterceptor implements Serializable {
   ...
   *
   * <p>Although the concrete validator is not necessarily serializable (and HV's implementation
   * indeed isn't) it is still alright to have it as non-transient field here. Upon passivation not
   * the validator itself will be serialized, but the proxy injected here, which in turn is
   * serializable.
   */
