 ...
 * This function adapts the Shamrock bytecode transformer API - which uses ASM - to use the Entity
 * Enhancement API of Hibernate ORM, which exposes a simple byte array.
 *
 * <p>N.B. For enhancement the hardcoded tool of choice is the Byte Buddy based enhancer. This is
 * not configurable, and we enforce the ORM environment to use the "noop" enhancer as we require all
...
public final class HibernateEntityEnhancer
  ...
  public HibernateEntityEnhancer() {
    BytecodeProvider provider =
        new org.hibernate.bytecode.internal.bytebuddy.BytecodeProviderImpl();
    DefaultEnhancementContext enhancementContext =
  ...
  private class HibernateEnhancingClassVisitor extends ClassVisitor {
    ...
    public void visitEnd() {
      ...
      final ClassWriter writer =
          (ClassWriter)
              this.cv; // safe cast: cv is the the ClassWriter instance we passed to the super
      // constructor
      // We need to convert the nice Visitor chain into a plain byte array to adapt to the Hibernate
      // ORM
      // enhancement API:
