...
public class HibernateOrmConfig {
  ...
  /**
   * Control how schema generation is happening in Hibernate ORM. Same as JPA's
  ...
  /**
   * To populate the database tables with data before the application loads, specify the location of
   * a load script. The location specified in this property is relative to the root of the
   * persistence unit.
   */
  ...
  public boolean isAnyPropertySet() {
    return dialect.isPresent()
        || schemaGeneration.isPresent()
        || showSql.isPresent()
        || sqlLoadScriptSource.isPresent();
  }
}
