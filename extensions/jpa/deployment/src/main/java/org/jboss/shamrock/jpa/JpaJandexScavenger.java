 ...
 *
 * <p>The output is then both consumed as plain list to use as a filter for which classes need to be
 * enhanced, collect them for storage in the JPADeploymentTemplate and registered for reflective
 * access. TODO some of these are going to be redundant?
 *
...
final class JpaJandexScavenger {
  ...
  private static final DotName MAPPED_SUPERCLASS =
  ...
  public KnownDomainObjects discoverModelAndRegisterForReflection() throws IOException {
    ...
    for (PersistenceUnitDescriptor pud : descriptors) {
      enlistExplicitClasses(
          indexView, domainObjectCollector, enumTypeCollector, pud.getManagedClassNames());
    }
  ...
  private static void enlistExplicitClasses(
      IndexView index,
      DomainObjectSet domainObjectCollector,
    ...
    for (String className : managedClassNames) {
      ...
      if (isInIndex) {
        ...
        // We do lipstick service by manually adding explicitly the <class> reference but not
  ...
  private static void enlistReturnType(
      IndexView index, DomainObjectSet domainObjectCollector, Set<String> enumTypeCollector) {
        ...
        switch (target.kind()) {
          ...
          default:
            throw new IllegalStateException(
                "[internal error] @Embedded placed on a unknown element: " + target);
        }
        addClassHierarchyToReflectiveList(
            index, domainObjectCollector, enumTypeCollector, jpaClassName);
      }
    }
  }

  private static void enlistJPAModelClasses(
      IndexView index,
      DomainObjectSet domainObjectCollector,
      Set<String> enumTypeCollector,
      DotName dotName) {
    ...
    if (jpaAnnotations != null && jpaAnnotations.size() > 0) {
      for (AnnotationInstance annotation : jpaAnnotations) {
        ...
        addClassHierarchyToReflectiveList(
            index, domainObjectCollector, enumTypeCollector, targetDotName);
  ...
  /**
   * Add the class to the reflective list with full method and field access. Add the superclasses
   * recursively as well as the interfaces.
   *
   * <p>TODO this approach fails if the Jandex index is not complete (e.g. misses somes interface or
   * super types) TODO should we also return the return types of all methods and fields? It could
   * container Enums for example.
   */
  private static void addClassHierarchyToReflectiveList(
      IndexView index,
      DomainObjectSet domainObjectCollector,
      Set<String> enumTypeCollector,
      DotName className) {
    ...
    if (classInfo == null) {
      if (className.equals(ClassType.OBJECT_TYPE.name())
          || className.toString().equals(Serializable.class.getName())) {
        return;
      } else {
        throw new IllegalStateException(
            "The Jandex index is not complete, missing: " + className.toString());
      }
    }
    ...
    // Capture this one (for various needs: Reflective access enablement, Hibernate enhancement, JPA
    // Template)
    domainObjectCollector.addEntity(className.toString());
    // add superclass recursively
    addClassHierarchyToReflectiveList(
        index, domainObjectCollector, enumTypeCollector, classInfo.superName());
    // add interfaces recursively
    for (DotName interfaceDotName : classInfo.interfaceNames()) {
      addClassHierarchyToReflectiveList(
          index, domainObjectCollector, enumTypeCollector, interfaceDotName);
    }
  }
