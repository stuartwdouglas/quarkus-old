...
import org.jboss.shamrock.jpa.runtime.boot.FastBootEntityManagerFactoryBuilder;
import org.jboss.shamrock.jpa.runtime.boot.registry.PreconfiguredServiceRegistryBuilder;
 ...
 * to include all code which could be triggered from using that: we need to be able to fully exclude
 * HibernatePersistenceProvider from the native image.
 */
final class FastBootHibernatePersistenceProvider implements PersistenceProvider {
  ...
  public EntityManagerFactory createEntityManagerFactory(
    ...
    final EntityManagerFactoryBuilder builder =
        getEntityManagerFactoryBuilderOrNull(persistenceUnitName, properties);
    if (builder == null) {
   ...
   * HibernatePersistenceProvider#getEntityManagerFactoryBuilderOrNull(String, Map, ClassLoader,
   * ClassLoaderService) Notable changes: - ignore the ClassLoaderService parameter to inject our
   * own custom implementation instead - verify the Map properties are not set (or fail as we can't
   * support runtime overrides) - don't try looking for ParsedPersistenceXmlDescriptor resources to
   * parse, just take the pre-parsed ones from the static final field - class annotations metadata
   * is also injected
   */
  @SuppressWarnings("rawtypes")
  private EntityManagerFactoryBuilder getEntityManagerFactoryBuilderOrNull(
    ...
    for (PersistenceUnitDescriptor persistenceUnit : units) {
      ...
      if (!matches) {
        log.debugf(
            "Excluding from consideration '%s' due to name mis-match", persistenceUnit.getName());
        continue;
      }
      ...
      StandardServiceRegistry standardServiceRegistry =
          rewireMetadataAndExtractServiceRegistry(configurationValues, recordedState);
  ...
  private StandardServiceRegistry rewireMetadataAndExtractServiceRegistry(
      Map<String, Object> configurationValues, RecordedState rs) {
    PreconfiguredServiceRegistryBuilder serviceRegistryBuilder =
        new PreconfiguredServiceRegistryBuilder(rs);
  ...
  @SuppressWarnings("rawtypes")
  public static String extractRequestedProviderName(
      PersistenceUnitDescriptor persistenceUnit, Map integration) {
    ...
    if (persistenceUnitRequestedProvider != null) {
      log.debugf(
          ...
          persistenceUnit.getName(), persistenceUnitRequestedProvider);
  ...
  private static String extractProviderName(PersistenceUnitDescriptor persistenceUnit) {
    ...
    return persistenceUnitRequestedProvider == null
        ? null
        : persistenceUnitRequestedProvider.trim();
  }
