...
public class JPADeploymentTemplate {
  ...
  public void enlistPersistenceUnit() {
    Logger.getLogger("org.jboss.shamrock.jpa")
        .debugf("List of entities found by Shamrock deployment:%n%s", entities);
  }
  ...
  public BeanContainerListener initMetadata(
      List<ParsedPersistenceXmlDescriptor> parsedPersistenceXmlDescriptors, Scanner scanner) {
    return new BeanContainerListener() {
      @Override
      public void created(BeanContainer beanContainer) {
        // this initializes the JPA metadata, and also sets the datasource if no connection URL has
        // been set and a DataSource
        // is available
