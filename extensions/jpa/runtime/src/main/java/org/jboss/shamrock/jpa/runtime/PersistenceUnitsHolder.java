...
public final class PersistenceUnitsHolder {
  ...
  /**
   * Initialize JPA for use in Shamrock. In a native image. This must be called from within a static
   * init method.
   *
   * <p>In general the <code>parsedPersistenceXmlDescriptors</code> will be provided by calling
   * {@link #loadOriginalXMLParsedDescriptors()} In Shamrock this is done in Shamrock's JPA
   * ResourceProcessor.
   *
   * <p>The scanner may be null to use the default scanner, or a custom scanner can be used to stop
   * Hibernate scanning. It is expected that the scanner will be provided by Shamrock via its hold
   * of Jandex info.
   *
  ...
  static void initializeJpa(
      List<ParsedPersistenceXmlDescriptor> parsedPersistenceXmlDescriptors, Scanner scanner) {
    final List<PersistenceUnitDescriptor> units =
        convertPersistenceUnits(parsedPersistenceXmlDescriptors);
