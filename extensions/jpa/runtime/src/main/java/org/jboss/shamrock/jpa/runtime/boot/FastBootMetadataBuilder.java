...
/**
 * Alternative to EntityManagerFactoryBuilderImpl so to have full control of how
 * MetadataBuilderImplementor is created, which configuration properties are supportable, custom
 * overrides, etc...
 */
public class FastBootMetadataBuilder {

  private static final EntityManagerMessageLogger LOG =
      messageLogger(FastBootMetadataBuilder.class);
  ...
  public FastBootMetadataBuilder(final PersistenceUnitDescriptor persistenceUnit, Scanner scanner) {
    ...
    this.metamodelBuilder =
        (MetadataBuilderImplementor) metadataSources.getMetadataBuilder(standardServiceRegistry);
    if (scanner != null) {
  ...
  private void addPUManagedClassNamesToMetadataSources(
      PersistenceUnitDescriptor persistenceUnit, MetadataSources metadataSources) {
  ...
  private MergedSettings mergeSettings(PersistenceUnitDescriptor persistenceUnit) {
    ...
    if (persistenceUnit.getJtaDataSource() != null) {
      mergedSettings.configurationValues.put(
          "hibernate.connection.datasource", persistenceUnit.getJtaDataSource());
    }
    // Protean specific!
  ...
  private void destroyServiceRegistry(MetadataImplementor fullMeta) {
    final AbstractServiceRegistryImpl serviceRegistry =
        (AbstractServiceRegistryImpl) metamodelBuilder.getBootstrapContext().getServiceRegistry();
    serviceRegistry.close();
    serviceRegistry.resetParent(null);
  }

  private MetadataImplementor trimBootstrapMetadata(MetadataImpl fullMeta) {
    MetadataImpl replacement =
        new MetadataImpl(
            fullMeta.getUUID(),
            ...
            fullMeta.getNamedNativeQueryMap(), // TODO // might contain references to
            ...
            fullMeta.getSqlResultSetMappingMap(), // TODO might contain NativeSQLQueryReturn (as
            // namedNativeQueryMap above)
            fullMeta.getNamedEntityGraphs(), // TODO //reference to *annotation* instance ! FIXME or
            // ignore feature?
            fullMeta.getSqlFunctionMap(), // ok
            fullMeta.getDatabase(), // Cleaned up: used to include references to
            // MetadataBuildingOptions, etc..
  ...
  private JtaPlatform extractJtaPlatform() {
    ...
    //        JtaPlatformResolver service = standardServiceRegistry.getService(
    // JtaPlatformResolver.class );
    //        return service.resolveJtaPlatform( this.configurationValues,
  ...
  private void applyTransactionProperties(StandardServiceRegistryBuilder ssrBuilder) {
    PersistenceUnitTransactionType txnType =
        PersistenceUnitTransactionTypeHelper.interpretTransactionType(
    ...
    if (hasTxStrategy) {
      ...
      if (txnType == PersistenceUnitTransactionType.JTA) {
        ssrBuilder.applySetting(
            TRANSACTION_COORDINATOR_STRATEGY, JtaTransactionCoordinatorBuilderImpl.class);
  ...
  private void configure(StandardServiceRegistry ssr, MergedSettings mergedSettings) {
    ...
    // apply id generators
    final Object idGeneratorStrategyProviderSetting =
    ...
    if (idGeneratorStrategyProviderSetting != null) {
      final IdentifierGeneratorStrategyProvider idGeneratorStrategyProvider =
          strategySelector.resolveStrategy(
              IdentifierGeneratorStrategyProvider.class, idGeneratorStrategyProviderSetting);
      final MutableIdentifierGeneratorFactory identifierGeneratorFactory =
          ssr.getService(MutableIdentifierGeneratorFactory.class);
      ...
      for (Map.Entry<String, Class<?>> entry :
          idGeneratorStrategyProvider.getStrategies().entrySet()) {
  ...
  protected void populate(
    ...
    final TypeContributorList typeContributorList =
        (TypeContributorList)
            configurationValues.remove(EntityManagerFactoryBuilderImpl.TYPE_CONTRIBUTORS);
  ...
  private void applyMetadataBuilderContributor() {
    ...
    Object metadataBuilderContributorSetting =
        configurationValues.get(EntityManagerFactoryBuilderImpl.METADATA_BUILDER_CONTRIBUTOR);
    ...
    if (metadataBuilderContributorSetting instanceof MetadataBuilderContributor) {
      ...
      final ClassLoaderService classLoaderService =
          standardServiceRegistry.getService(ClassLoaderService.class);

      metadataBuilderContributorImplClass =
          classLoaderService.classForName((String) metadataBuilderContributorSetting);
    } else {
    ...
    if (metadataBuilderContributorImplClass != null) {
      try {
        ...
        throw new IllegalArgumentException(
            "The MetadataBuilderContributor class ["
                + metadataBuilderContributorImplClass
