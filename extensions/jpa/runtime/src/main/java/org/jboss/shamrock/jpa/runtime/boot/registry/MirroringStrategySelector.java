...
package org.jboss.shamrock.jpa.runtime.boot.registry;
...
/**
 * FIXME: Apparently only used by extension points we don't support yet. The idea is to intercept
 * and record the answers from the original StrategySelectorImpl and have this parrot the same
 * answers.
 */
public class MirroringStrategySelector implements StrategySelector {

  @Override
  public <T> void registerStrategyImplementor(
      Class<T> aClass, String s, Class<? extends T> aClass1) {}
  ...
  public <T> T resolveStrategy(
      Class<T> aClass, Object o, Callable<T> callable, StrategyCreator<T> strategyCreator) {
    return null;
  }
