...
/**
 * Replacement implementation of org.hibernate.jmx.internal.JmxServiceInitiator MBeans are currently
 * too troublesome in GraalVM, so enforce disabling its usage.
 */
  ...
  public JmxService initiateService(Map configurationValues, ServiceRegistryImplementor registry) {
    if (ConfigurationHelper.getBoolean(AvailableSettings.JMX_ENABLED, configurationValues, false)) {
      Logger.getLogger(DisabledJMXInitiator.class)
          .warn(
              "Enabling JMX is not allowed in Protean: forcefully disabled. Ignoring property:"
