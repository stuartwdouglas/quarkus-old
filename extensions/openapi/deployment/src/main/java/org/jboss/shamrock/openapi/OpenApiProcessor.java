...
public class OpenApiProcessor {
  ...
  List<HotDeploymentConfigFileBuildItem> configFiles() {
    return Stream.of(
            META_INF_OPENAPI_YAML,
            WEB_INF_CLASSES_META_INF_OPENAPI_YAML,
            META_INF_OPENAPI_YML,
            WEB_INF_CLASSES_META_INF_OPENAPI_YML,
            META_INF_OPENAPI_JSON,
            WEB_INF_CLASSES_META_INF_OPENAPI_JSON)
  ...
  ServletBuildItem servlet() {
    ServletBuildItem servletBuildItem =
        new ServletBuildItem("openapi", OpenApiServlet.class.getName());
  ...
  List<AdditionalBeanBuildItem> beans() {
    return Arrays.asList(
        new AdditionalBeanBuildItem(OpenApiServlet.class),
  ...
  public BeanContainerListenerBuildItem build(
      OpenApiDeploymentTemplate template,
    ...
    OpenAPI sm =
        generateStaticModel(
            resourcePath == null ? null : resourcePath.path,
  ...
  public OpenAPI generateAnnotationModel(IndexView indexView, JaxrsConfig jaxrsConfig) {
    ...
    return new OpenApiAnnotationScanner(
            openApiConfig, indexView, Arrays.asList(new RESTEasyExtension(jaxrsConfig, indexView)))
        .scan();
  }
  ...
  private Result findStaticModel(ApplicationArchivesBuildItem archivesBuildItem) {
    ...
    if (resourcePath == null) {
      resourcePath =
          archivesBuildItem.getRootArchive().getChildPath(WEB_INF_CLASSES_META_INF_OPENAPI_YAML);
    }
    if (resourcePath == null) {
    ...
    if (resourcePath == null) {
      resourcePath =
          archivesBuildItem.getRootArchive().getChildPath(WEB_INF_CLASSES_META_INF_OPENAPI_YML);
    }
    if (resourcePath == null) {
    ...
    if (resourcePath == null) {
      resourcePath =
          archivesBuildItem.getRootArchive().getChildPath(WEB_INF_CLASSES_META_INF_OPENAPI_JSON);
      format = OpenApiSerializer.Format.JSON;
    }
    ...
    return new Result(
        format,
        archivesBuildItem.getRootArchive().getArchiveRoot().relativize(resourcePath).toString());
  }
