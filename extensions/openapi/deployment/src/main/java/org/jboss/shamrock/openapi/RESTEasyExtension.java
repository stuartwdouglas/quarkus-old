...
public class RESTEasyExtension extends DefaultAnnotationScannerExtension {
  ...
  public static final DotName DOTNAME_QUERY_PARAM =
      DotName.createSimple("org.jboss.resteasy.annotations.jaxrs.QueryParam");
  public static final DotName DOTNAME_FORM_PARAM =
      DotName.createSimple("org.jboss.resteasy.annotations.jaxrs.FormParam");
  public static final DotName DOTNAME_COOKIE_PARAM =
      DotName.createSimple("org.jboss.resteasy.annotations.jaxrs.CookieParam");
  public static final DotName DOTNAME_PATH_PARAM =
      DotName.createSimple("org.jboss.resteasy.annotations.jaxrs.PathParam");
  public static final DotName DOTNAME_HEADER_PARAM =
      DotName.createSimple("org.jboss.resteasy.annotations.jaxrs.HeaderParam");
  public static final DotName DOTNAME_MATRIX_PARAM =
      DotName.createSimple("org.jboss.resteasy.annotations.jaxrs.MatrixParam");
  private static final DotName DOTNAME_PROVIDER = DotName.createSimple("javax.ws.rs.ext.Provider");
  private static final DotName DOTNAME_ASYNC_RESPONSE_PROVIDER =
      DotName.createSimple("org.jboss.resteasy.spi.AsyncResponseProvider");
  ...
  private void scanAsyncResponseProvidersFromServices() {
    try {
      Class<?> asyncResponseProvider =
          Class.forName("org.jboss.resteasy.spi.AsyncResponseProvider");
      // can't use the ServiceLoader API because Providers is not an interface
      Enumeration<URL> resources =
          getClass().getClassLoader().getResources("META-INF/services/javax.ws.rs.ext.Providers");
  ...
  private void scanAsyncResponseProvidersFromClassName(
      Class<?> asyncResponseProviderClass, String name) {
    try {
          ...
          if (type instanceof java.lang.reflect.ParameterizedType) {
            ...
            if (pType.getRawType().equals(asyncResponseProviderClass)
              ...
              if (asyncType instanceof java.lang.reflect.ParameterizedType)
                asyncTypeName =
                    ((java.lang.reflect.ParameterizedType) asyncType).getRawType().getTypeName();
              else asyncTypeName = asyncType.getTypeName();
  ...
  public JaxRsParameterInfo getMethodParameterJaxRsInfo(MethodInfo method, int idx) {
    AnnotationInstance jaxRsAnno =
        JandexUtil.getMethodParameterAnnotation(method, idx, DOTNAME_PATH_PARAM);
    if (jaxRsAnno != null) {
      JaxRsParameterInfo info = new JaxRsParameterInfo();
      info.in = In.PATH;
      info.name = JandexUtil.stringValue(jaxRsAnno, OpenApiConstants.PROP_VALUE);
      if (info.name == null) info.name = method.parameterName(idx);
      return info;
    }
  ...
  public In parameterIn(MethodParameterInfo paramInfo) {
    ...
    List<AnnotationInstance> annotations =
        JandexUtil.getParameterAnnotations(method, paramPosition);
  ...
  public Type resolveAsyncType(Type type) {
    if (type.kind() == Type.Kind.PARAMETERIZED_TYPE && asyncTypes.contains(type.name())) {
      ...
      if (pType.arguments().size() == 1) return pType.arguments().get(0);
    }
    return super.resolveAsyncType(type);
  }

  @Override
  public void processJaxRsApplications(
      OpenApiAnnotationScanner scanner, Collection<ClassInfo> applications) {
    if (applications.isEmpty()) scanner.setCurrentAppPath(defaultPath);
  }
}
