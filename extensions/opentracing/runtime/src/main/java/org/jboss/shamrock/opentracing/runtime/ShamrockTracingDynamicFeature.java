...
public class ShamrockTracingDynamicFeature implements DynamicFeature {
  ...
  private static final Logger logger =
      Logger.getLogger(io.smallrye.opentracing.SmallRyeTracingDynamicFeature.class.getName());

  private final ServerTracingDynamicFeature delegate;

  public ShamrockTracingDynamicFeature() {
    Config config = ConfigProvider.getConfig();
    Optional<String> skipPattern =
        config.getOptionalValue("mp.opentracing.server.skip-pattern", String.class);
    Optional<String> operationNameProvider =
    ...
    if (operationNameProvider.isPresent()) {
      if ("http-path".equalsIgnoreCase(operationNameProvider.get())) {
        ...
        logger.warning(
            "Provided operation name does not match http-path or class-method. Using default class-method.");
      }
    }
