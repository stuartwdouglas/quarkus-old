...
public class QuartzScheduler implements Scheduler {
  ...
  @Inject SchedulerConfiguration schedulerConfig;
  ...
  public void startTimer(long delay, Runnable action) {
    if (running.get()) {
      ...
      JobDetail job =
          JobBuilder.newJob(TimerJob.class).withIdentity(name, Scheduler.class.getName()).build();
  ...
  void start(@Observes StartupEvent startupEvent) {
    if (running.compareAndSet(false, true)) {
      ...
      try {
        // TODO: leverage shamrock config - these values are just copied from the default
        // quartz.properties
        ...
        for (Entry<String, List<Scheduled>> entry : schedulerConfig.getSchedules().entrySet()) {
          ...
          for (Scheduled scheduled : entry.getValue()) {
            ...
            JobBuilder jobBuilder =
                JobBuilder.newJob(InvokerJob.class)
            ...
            if (scheduled.delay() > 0) {
              triggerBuilder.startAt(
                  new Date(
                      Instant.now()
                          .plusMillis(scheduled.delayUnit().toMillis(scheduled.delay()))
                          .toEpochMilli()));
            }
            scheduler.scheduleJob(jobBuilder.build(), triggerBuilder.build());
            LOGGER.debugf(
                "Scheduled business method %s with config %s",
  ...
  class InvokerJob implements Job {
    ...
    public void execute(JobExecutionContext context) throws JobExecutionException {
      ...
      String invokerClass =
          context.getJobDetail().getJobDataMap().getString(SchedulerDeploymentTemplate.INVOKER_KEY);
      invokers
          .computeIfAbsent(invokerClass, schedulerConfig::createInvoker)
          .invoke(
