...
public class SchedulerDeploymentTemplate {
  ...
  public void registerSchedules(List<Map<String, Object>> configurations, BeanContainer container) {
    ...
    for (Map<String, Object> config : configurations) {
      ...
      List<Map<String, Object>> schedulesConfig =
          (List<Map<String, Object>>) config.get(SCHEDULES_KEY);
      ...
      schedulerConfig.register(
          config.get(INVOKER_KEY).toString(), config.get(DESC_KEY).toString(), schedules);
    }
  }
}
