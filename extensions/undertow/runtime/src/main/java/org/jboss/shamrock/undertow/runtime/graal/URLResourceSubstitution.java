 ...
 *
 * <p>This class is a bit of a hack to allow static resource to work correctly with Substrate.
 *
 * <p>The class path is iterated at image build time, and any web resources have their content
 * length and last modified times stored in a Map.
 *
 * <p>This seems like a simple enough solution for now, but hopefully we can do something better in
 * the future.
 *
 * <p>Another possibility is just not supporting embedded resources, and requiring resources to be
 * served from the file system (or extracted tmp dir)
 */
@TargetClass(className = "io.undertow.server.handlers.resource.URLResource")
final class URLResourceSubstitution {

  @Alias private boolean connectionOpened = false;
  @Alias private Date lastModified;
  @Alias private Long contentLength;
          ...
          try (JarFile file = new JarFile(res.getPath())) {
            ...
            while (e.hasMoreElements()) {
              ...
              if (entry.getName().startsWith(META_INF_RESOURCES)) {
                map.put(
                    entry.getName().substring(META_INF_RESOURCES.length()),
