...
final class LoggerProviders {
  ...
  private static LoggerProvider findProvider() {
    // Since the impl classes refer to the back-end frameworks directly, if this classloader can't
    // find the target
    // log classes, then it doesn't really matter if they're possibly available from the TCCL
    // because we won't be
    // able to find it anyway
    final ClassLoader cl = LoggerProviders.class.getClassLoader();
    try {
      // Check the system property
      final String loggerProvider =
    ...
    try {
      // MUST try Log4j 2.x BEFORE Log4j 1.x because Log4j 2.x also passes Log4j 1.x test in some
      // circumstances
  ...
  private static LoggerProvider tryLog4j2(final ClassLoader cl, final String via)
    ...
    // if Log4j 2 has a bad implementation that doesn't extend AbstractLogger, we won't know until
    // getting the first logger throws an exception
    logProvider(provider, via);
    return provider;
  }
  ...
  private static LoggerProvider tryLog4j(final ClassLoader cl, final String via)
    ...
    // JBLOGGING-65 - slf4j can disguise itself as log4j.  Test for a class that slf4j doesn't
    // provide.
  ...
  private static LoggerProvider tryJBossLogManager(final ClassLoader cl, final String via)
    ...
    if (logManagerClass == Class.forName("org.jboss.logmanager.LogManager", false, cl)
        && Class.forName("org.jboss.logmanager.Logger$AttachmentKey", true, cl).getClassLoader()
            == logManagerClass.getClassLoader()) {
