...
/**
 * A simple utility to resolve the default locale to use for internationalized loggers and message
 * bundles.
 *
...
class LoggingLocale {
  ...
  /**
   * Attempts to create a {@link Locale} based on the {@code org.jboss.logging.locale} system
   * property. If the value is not defined the {@linkplain Locale#getDefault() default locale} will
   * be used.
   *
   * <p>The value should be in the <a href="https://tools.ietf.org/html/bcp47">BCP 47</a> format.
   *
   * <p><strong>Note:</strong> Currently this uses a custom parser to attempt to parse the BCP 47
   * format. This will be changed to use the {@code Locale.forLanguageTag()} once a move to JDK 7.
   * Currently only the language, region and variant are used to construct the locale.
   *
  ...
  private static Locale getDefaultLocale() {
    ...
    // When we upgrade to Java 7 we can use the Locale.forLanguageTag(locale) which will reliably
    // parse the
  ...
  private static Locale forLanguageTag(final String locale) {
    ...
    // Next should be the variant and we will just use the first one found, all other parts will be
    // ignored
