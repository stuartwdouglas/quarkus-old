...
/**
 * Utility for snapshot/copy-on-write arrays. To use these methods, two things are required: an
 * immutable array stored on a volatile field, and an instance of {@link
 * java.util.concurrent.atomic.AtomicReferenceFieldUpdater AtomicReferenceFieldUpdater} which
 * corresponds to that field. Some of these methods perform multi-step operations; if the array
 * field value is changed in the middle of such an operation, the operation is retried. To avoid
 * spinning, in some situations it may be advisable to hold a write lock to prevent multiple
 * concurrent updates.
 *
...
final class AtomicArray<T, V> {
   ...
   */
  public static <T, V> AtomicArray<T, V> create(
      AtomicReferenceFieldUpdater<T, V[]> updater, Class<V> componentType) {
  ...
  /**
   * Atomically replace the array with a new array which is one element longer, and which includes
   * the given value.
   *
   * @param instance the instance holding the field
  ...
  /**
   * Atomically replace the array with a new array which is one element longer, and which includes
   * the given value, if the value is not already present within the array. This method does a
   * linear search for the target value.
   *
   * @param instance the instance holding the field
   * @param value the updated value
   * @param identity {@code true} if comparisons should be done using reference identity, or {@code
   *     false} to use the {@code equals()} method
  ...
  /**
   * Atomically replace the array with a new array which does not include the first occurrance of
   * the given value, if the value is present in the array.
   *
   * @param instance the instance holding the field
   * @param value the updated value
   * @param identity {@code true} if comparisons should be done using reference identity, or {@code
  ...
  /**
   * Atomically replace the array with a new array which does not include any occurrances of the
   * given value, if the value is present in the array.
   *
   * @param instance the instance holding the field
   * @param value the updated value
   * @param identity {@code true} if comparisons should be done using reference identity, or {@code
  ...
  /**
   * Remove a value to a sorted array. Does not check for duplicates. If there are multiple
   * occurrances of a value, there is no guarantee as to which one is removed.
   *
   * @param instance the instance holding the field
   ...
   * @return {@code true} if the value was updated or {@code false} if the expected value did not
   *     match
   */
