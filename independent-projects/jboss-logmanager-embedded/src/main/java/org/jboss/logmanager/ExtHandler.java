...
/**
 * An extended logger handler. Use this class as a base class for log handlers which require {@code
...
public abstract class ExtHandler extends Handler implements FlushableCloseable {
  ...
  /**
   * The sub-handlers for this handler. May only be updated using the {@link #handlersUpdater}
  ...
  protected static final AtomicArray<ExtHandler, Handler> handlersUpdater =
      AtomicArray.create(
          AtomicReferenceFieldUpdater.newUpdater(ExtHandler.class, Handler[].class, "handlers"),
          Handler.class);
   ...
   *
   * <p>The logging request was made initially to a Logger object, which initialized the LogRecord
   * and forwarded it here.
   *
   * <p>The {@code ExtHandler} is responsible for formatting the message, when and if necessary. The
  ...
  /**
   * Do the actual work of publication; the record will have been filtered already. The default
   * implementation does nothing except to flush if the {@code autoFlush} property is set to {@code
   * true}; if this behavior is to be preserved in a subclass then this method should be called
   * after the record is physically written.
   *
   * @param record the log record to publish
   */
  ...
  /**
   * Get a copy of the sub-handlers array. Since the returned value is a copy, it may be freely
   * modified.
   *
  ...
  /**
   * Indicates whether or not children handlers should be closed when this handler is {@linkplain
   * #close() closed}.
   *
   * @return {@code true} if the children handlers should be closed when this handler is closed,
   *     {@code false} if children handlers should not be closed when this handler is closed
   */
  ...
  /**
   * Sets whether or not children handlers should be closed when this handler is {@linkplain
   * #close() closed}.
   *
   * @param closeChildren {@code true} if all children handlers should be closed when this handler
   *     is closed, {@code false} if children handlers will <em>not</em> be closed when this handler
   *     is closed
   */
  ...
  /**
   * Indicates whether or not the {@linkplain #getFormatter() formatter} associated with this
   * handler or a formatter from a {@linkplain #getHandlers() child handler} requires the caller to
   * be calculated.
   *
   * <p>Calculating the caller on a {@linkplain ExtLogRecord log record} can be an expensive
   * operation. Some handlers may be required to copy some data from the log record, but may not
   * need the caller information. If the {@linkplain #getFormatter() formatter} is a {@link
   * ExtFormatter} the {@link ExtFormatter#isCallerCalculationRequired()} is used to determine if
   * calculation of the caller is required.
   *
   * @return {@code true} if the caller should be calculated, otherwise {@code false} if it can be
   *     skipped
