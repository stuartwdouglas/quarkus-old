...
/**
 * A HashMap that is optimized for fast shallow copies. If the copy-ctor is passed another
 * FastCopyHashMap, or clone is called on this map, the shallow copy can be performed using little
 * more than a single array copy. In order to accomplish this, immutable objects must be used
 * internally, so update operations result in slightly more object churn than <code>HashMap</code>.
 *
 * <p>Note: It is very important to use a smaller load factor than you normally would for HashMap,
 * since the implementation is open-addressed with linear probing. With a 50% load-factor a get is
 * expected to return in only 2 probes. However, a 90% load-factor is expected to return in around
 * 50 probes.
 *
 * @author Jason T. Greene
 */
class FastCopyHashMap<K, V> extends AbstractMap<K, V>
  ...
  public FastCopyHashMap(Map<? extends K, ? extends V> map) {
    if (map instanceof FastCopyHashMap) {
      FastCopyHashMap<? extends K, ? extends V> fast =
          (FastCopyHashMap<? extends K, ? extends V>) map;
  ...
  public boolean containsValue(Object value) {
    for (Entry<K, V> e : table) if (e != null && eq(value, e.value)) return true;
  ...
  public void printDebugStats() {
    ...
    System.out.println(
        " Optimal:         " + optimal + " (" + (float) optimal * 100 / total + "%)");
  ...
  private class EntryIterator extends FastCopyHashMapIterator<Map.Entry<K, V>> {
    private class WriteThroughEntry extends SimpleEntry<K, V> {
      ...
      public V setValue(V value) {
        if (table != FastCopyHashMap.this.table) FastCopyHashMap.this.put(getKey(), value);
  ...
  protected static class SimpleEntry<K, V> implements Map.Entry<K, V> {
    ...
    public int hashCode() {
      return (key == null ? 0 : hash(key)) ^ (value == null ? 0 : hash(value));
    }
