...
public final class Logger extends java.util.logging.Logger implements Serializable {
  ...
  public static Logger getLogger(final String name) {
    try {
      ...
      throw new IllegalStateException(
          "The LogManager was not properly installed (you must set the \"java.util.logging.manager\" system property to \""
  ...
  /**
   * {@inheritDoc} This implementation grabs a lock, so that only one thread may update the log
   * level of any logger at a time, in order to allow readers to never block (though there is a
   * window where retrieving the log level reflects an older effective level than the actual level).
   */
   ...
   * @return {@code true} if the handler list was updated or {@code false} if the current handlers
   *     did not match the expected handlers list
   * @throws SecurityException if a security manager exists and if the caller does not have {@code
   *     LoggingPermission(control)}
   */
   ...
   *
   * <p>Setting this value to {@code false} has the same behaviour as {@linkplain
  ...
  public void log(
      ...
      final String bundleName,
      final ExtLogRecord.FormatStyle style,
      final Object[] params,
      final Throwable t) {
    if (level == null
        || fqcn == null
  ...
  /**
   * Do the logging with no level checks (they've already been done). Creates an extended log record
   * if the provided record is not one.
   *
