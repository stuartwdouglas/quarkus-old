...
/**
 * A node in the tree of logger names. Maintains weak references to children and a strong reference
 * to its parent.
 */
final class LoggerNode {
  ...
  /**
   * The handlers for this logger. May only be updated using the {@link #handlersUpdater} atomic
   * updater. The array instance should not be modified (treat as immutable).
   */
  ...
  private static final AtomicArray<LoggerNode, Handler> handlersUpdater =
      AtomicArray.create(
          AtomicReferenceFieldUpdater.newUpdater(LoggerNode.class, Handler[].class, "handlers"),
          Handler.class);

  /**
   * The actual level. May only be modified when the context's level change lock is held; in
   * addition, changing this field must be followed immediately by recursively updating the
   * effective loglevel of the child tree.
   */
  private volatile java.util.logging.Level level;
  /**
   * The effective level. May only be modified when the context's level change lock is held; in
   * addition, changing this field must be followed immediately by recursively updating the
   * effective loglevel of the child tree.
   */
  ...
  private LoggerNode(LogContext context, LoggerNode parent, String nodeName) {
    ...
    effectiveLevel =
        max(effectiveMinLevel, level == null ? parent.effectiveLevel : level.intValue());
    handlersUpdater.set(this, configurator.getHandlersOf(fullName));
    this.context = context;
    children = new CopyOnWriteMap<String, LoggerNode>();
  }
  ...
  /**
   * Update the effective level if it is inherited from a parent. Must only be called while the
   * logmanager's level change lock is held.
   *
  ...
  /**
   * Checks the filter to see if the record is loggable. If the {@link #getUseParentFilters()} is
   * set to {@code true} the parent loggers are checked.
   *
