...
public final class SubstituteFilter implements Filter {
   ...
   * @param replaceAll {@code true} if all occurrances should be replaced; {@code false} if only the
   *     first occurrance
   */
  public SubstituteFilter(
      final Pattern pattern, final String replacement, final boolean replaceAll) {
   ...
   */
  public SubstituteFilter(
      final String patternString, final String replacement, final boolean replaceAll) {
   ...
   *
   * <p>The {@link FormatStyle format style} will always be set to {@link FormatStyle#NO_FORMAT} as
   * the formatted message will be the one used in the replacement.
   *
