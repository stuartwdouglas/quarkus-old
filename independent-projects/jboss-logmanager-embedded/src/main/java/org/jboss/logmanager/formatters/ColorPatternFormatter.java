...
public class ColorPatternFormatter extends PatternFormatter {
  ...
  private String colorizePlain(final String str) {
    return ColorUtil.endFgColor(
            ColorUtil.startFgColor(new StringBuilder(), isTrueColor(), 0xff, 0xff, 0xff)
                .append(str))
        .toString();
  }
  ...
  static final class LevelColorStep implements FormatStep {
    ...
    public void render(
        final Formatter formatter, final StringBuilder builder, final ExtLogRecord record) {
      final int level =
          Math.max(Math.min(record.getLevel().intValue(), LARGEST_LEVEL), SMALLEST_LEVEL)
              - SMALLEST_LEVEL;
...
final class Target_ColorPatternFormatter {
  ...
  @Delete static boolean trueColor = false;
