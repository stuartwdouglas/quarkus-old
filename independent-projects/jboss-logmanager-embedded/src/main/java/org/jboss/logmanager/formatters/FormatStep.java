...
public interface FormatStep {
  ...
  /**
   * Emit an estimate of the length of data which this step will produce. The more accurate the
   * estimate, the more likely the format operation will be performant.
   *
   * @return an estimate
   */
  int estimateLength();
  ...
  /**
   * An enumeration of the types of items that can be rendered. Note that this enumeration may be
   * expanded in the future, so unknown values should be handled gracefully as if {@link #GENERIC}
   * were used.
   */
  enum ItemType {
