...
class StackTraceFormatter {
   ...
   */
  static void renderStackTrace(
      final StringBuilder builder, final Throwable t, final int suppressedDepth) {
  ...
  private void renderStackTrace(
      final StackTraceElement[] parentStack,
      final Throwable child,
      final String caption,
      final String prefix) {
    if (seen.contains(child)) {
      builder.append("\t[CIRCULAR REFERENCE:").append(child).append(']');
      newLine();
    } else {
      seen.add(child);
      ...
      builder.append(prefix).append(caption).append(child);
      newLine();
      for (int i = 0; i <= m; i++) {
  ...
  private void renderTrivial(final String prefix, final StackTraceElement element) {
    builder.append(prefix).append("\tat ").append(element);
    newLine();
  }
