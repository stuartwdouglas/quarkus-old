 ...
 *
 * <p>Note that including details can be expensive in terms of calculating the caller.
 *
 * <p>By default the {@linkplain #setRecordDelimiter(String) record delimiter} is set to {@code \n}.
 *
...
public abstract class StructuredFormatter extends ExtFormatter {
  ...
  public enum ExceptionOutputType {
    /**
     * The cause, if present, will be an array of stack trace elements. This will include suppressed
     * exceptions and the {@linkplain Throwable#getCause() cause} of the exception.
     */
    DETAILED,
    /**
     * The cause, if present, will be a string representation of the stack trace in a {@code
     * stackTrace} property. The property value is a string created by {@link
  ...
  private StructuredFormatter(final Map<Key, String> keyOverrides, final String keyOverridesValue) {
    ...
    this.keyOverrides =
        (keyOverrides == null ? Collections.emptyMap() : new EnumMap<>(keyOverrides));
    ...
    try {
      ...
      generator
          .add(
              ...
              dateTimeFormatter.format(Instant.ofEpochMilli(record.getMillis())))
      ...
      if (details) {
        generator
            .add(getKey(Key.SOURCE_CLASS_NAME), record.getSourceClassName())
   ...
   *
   * @return a string representation of the key overrides or {@code null} if no overrides were
   *     configured
   */
   ...
   * and may be {@code null} if no end of record character is desired.
   *
  ...
  /**
   * Sets the value to be used to indicate the end of a record. If set to {@code null} no delimiter
   * will be used at the end of the record.
   *
  ...
  /**
   * Sets the pattern to use when formatting the date. The pattern must be a valid {@link
   * java.time.format.DateTimeFormatter#ofPattern(String)} pattern.
   *
   * <p>If the pattern is {@code null} a default {@linkplain DateTimeFormatter#ISO_OFFSET_DATE_TIME
   * formatter} will be used. The {@linkplain #setZoneId(String) zone id} will always be appended to
   * the formatter. By default the zone id will default to the {@linkplain ZoneId#systemDefault()
   * systems zone id}.
   *
  ...
  /**
   * Returns the current zone id used for the {@linkplain #getDateTimeFormatter() date and time
   * formatter}.
   *
  ...
  /**
   * Sets the {@link ZoneId} to use when formatting the date and time from the {@link
   * java.util.logging.LogRecord}.
   *
   * <p>The rules of the id must conform to the rules specified on {@link ZoneId#of(String)}.
   *
   * @param zoneId the zone id or {@code null} to use the {@linkplain ZoneId#systemDefault() system
   *     default}
   ...
   *
   * <p>Printing the details can be expensive as the values are retrieved from the caller. The
   * details include the source class name, source file name, source method name and source line
   * number.
   *
  ...
  /**
   * Set the output type for exceptions. The default is {@link ExceptionOutputType#DETAILED
   * DETAILED}.
   *
   * @param exceptionOutputType the desired output type, if {@code null} {@link
   *     ExceptionOutputType#DETAILED} is used
   */
  ...
  /**
   * Checks the exception output type and determines if formatted output should be written. The
   * formatted output is equivalent to {@link Throwable#printStackTrace()}.
   *
  ...
  private void addException(
      final Generator generator, final Throwable throwable, final Map<Throwable, Integer> seen)
      throws Exception {
    if (throwable == null) {
      return;
    }
  ...
  private void addStackTraceElements(final Generator generator, final StackTraceElement[] elements)
      throws Exception {
  ...
  protected interface Generator {
     ...
     *
     * <p>By default this processes the map and uses {@link #add(String, String)} to add entries.
     *
     ...
     *
     * <p>If the {@link #wrapArrays()} returns {@code false} the key may be {@code null} and
     * implementations should handle this.
     *
     * @param key they key for the object, or {@code null} if this object was {@linkplain
     *     #startArray(String) started in an array} and the {@link #wrapArrays()} is {@code false}
     * @return the generator
     * @throws Exception if an error occurs while adding the data
     */
    ...
    /**
     * Writes the start of an array. This defaults to {@link #startObject(String)} for convenience
     * of generators that don't have a specific type for arrays.
     *
    ...
    /**
     * Writes an end for an array. This defaults to {@link #endObject()} for convenience of
     * generators that don't have a specific type for arrays.
     *
     * @return the generator
     * @throws Exception if an error occurs while adding the data
     */
     ...
     * Writes an attribute.
     *
     * <p>By default this uses the {@link #add(String, int)} method to add the attribute. If a
     * formatter requires special handling for attributes, for example an attribute on an XML
     * element, this method can be overridden.
     *
     ...
     * Writes an attribute.
     *
     * <p>By default this uses the {@link #add(String, String)} method to add the attribute. If a
     * formatter requires special handling for attributes, for example an attribute on an XML
     * element, this method can be overridden.
     *
     * @param name the name of the attribute
    ...
    /**
     * Indicates whether or not elements in an array should be wrapped or not. The default is {@code
     * false}.
     *
