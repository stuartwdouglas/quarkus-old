...
public interface ClientSocketFactory {
   ...
   *
   * <p>The default implementation simply returns {@code new InetSocketAddress(getAddress(),
   * getPort())}.
   *
  ...
  /**
   * Creates a new default implementation of the factory which uses {@link
   * SocketFactory#getDefault()} for TCP sockets and {@code new DatagramSocket()} for UDP sockets.
   *
  ...
  /**
   * Creates a new default implementation of the factory which uses the provided {@linkplain
   * SocketFactory#createSocket(InetAddress, int) socket factory} to create TCP connections and
   ...
   * @return the client socket factory
   */
  static ClientSocketFactory of(
      final SocketFactory socketFactory, final InetAddress address, final int port) {
    if (address == null || port < 0) {
      throw new IllegalArgumentException(
          String.format(
              "The address cannot be null (%s) and the port must be a positive integer (%d)",
              address, port));
    }
