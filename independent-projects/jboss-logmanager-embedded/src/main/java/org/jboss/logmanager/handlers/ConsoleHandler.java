...
public class ConsoleHandler extends OutputStreamHandler {
      ...
      new ErrorManager() {
        public void error(final String msg, final Exception ex, final int code) {
          final ExtLogRecord record =
              new ExtLogRecord(
                  Level.ERROR,
                  "Failed to publish log record (%s[%d]): %s",
          ...
          record.setParameters(
              new Object[] {
                codeStr, Integer.toString(code), msg,
              });
          record.setThrown(ex);
          publish(record);
        }
  ...
  public void setTarget(Target target) {
    final Target t =
        (target == null ? console == null ? Target.SYSTEM_OUT : Target.CONSOLE : target);
    switch (t) {
      case SYSTEM_OUT:
        setOutputStream(wrap(out));
        break;
  ...
  /**
   * Get the local error manager. This is an error manager that will publish errors to this console
   * handler. The console handler itself should not use this error manager.
   *
  ...
  private static Writer wrap(final Writer writer) {
    return writer == null
        ? null
        : writer instanceof UncloseableWriter ? writer : new UncloseableWriter(writer);
  }
