...
/**
 * A handler that queues messages until it's at least one child handler is {@linkplain
 * #addHandler(Handler) added} or {@linkplain #setHandlers(Handler[]) set}. If the children handlers
 * are {@linkplain #clearHandlers() cleared} then the handler is no longer considered activated and
 * messages will once again be queued.
 *
...
public class DelayedHandler extends ExtHandler {
  ...
  public final void close() throws SecurityException {
    synchronized (this) {
      if (!logRecords.isEmpty()) {
        ...
        StandardOutputStreams.printError(
            "The DelayedHandler was closed before any children handlers were "
   ...
   * {@inheritDoc}
   *
   * <p>Note that once this is invoked the handler will be activated and the messages will no longer
   * be queued. If more than one child handler is required the {@link #setHandlers(Handler[])}
   * should be used.
   *
   ...
   * {@inheritDoc}
   *
   * <p>Note that if the last child handler is removed the handler will no longer be activated and
   * the messages will again be queued.
   *
   * @see #clearHandlers()
   */
  @Override
   ...
   * {@inheritDoc}
   *
   * <p>Note that once this is invoked the handler will no longer be activated and messages will
   * again be queued.
   *
   ...
   * {@inheritDoc}
   *
   * <p>This can be overridden to always require the caller calculation by setting the {@link
   * #setCallerCalculationRequired(boolean)} value to {@code true}.
   *
  ...
  /**
   * Sets whether or not {@linkplain ExtLogRecord#copyAll() caller information} will be required
   * when formatting records.
   *
   * <p>If set to {@code true} the {@linkplain ExtLogRecord#copyAll() caller information} will be
   * calculated for each record that is placed in the queue. A value of {@code false} means the
   * {@link super#isCallerCalculationRequired()} will be used.
   *
   * <p>Note that the caller information is only attempted to be calculated when the handler has not
   * been activated. Once activated it's up to the {@linkplain #getHandlers() children handlers} to
   * determine how the record is processed.
   *
   * @param callerCalculationRequired {@code true} if the {@linkplain ExtLogRecord#copyAll() caller
   *     information} should always be calculated before the record is being placed in the queue
   */
