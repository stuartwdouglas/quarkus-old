 ...
 *
 * <p>The time interval is determined by the content of the suffix string which is passed in to
 * {@link #setSuffix(String)}.
 *
 * <p>The size interval is determined by the value passed in the {@link #setRotateSize(long)}.
 *
...
public class PeriodicSizeRotatingFileHandler extends PeriodicRotatingFileHandler {
   ...
   * @throws java.io.FileNotFoundException if the file could not be found on open
   */
  public PeriodicSizeRotatingFileHandler(final String fileName, final boolean append)
   ...
   * @throws java.io.FileNotFoundException if the file could not be found on open
   */
  public PeriodicSizeRotatingFileHandler(final File file, final String suffix)
      throws FileNotFoundException {
    super(file, suffix);
  }
   ...
   */
  public PeriodicSizeRotatingFileHandler(
      final File file, final String suffix, final long rotateSize, final int maxBackupIndex)
      throws FileNotFoundException {
    super(file, suffix);
  ...
  protected void preWrite(final ExtLogRecord record) {
    ...
    if (currentSize > rotateSize && maxBackupIndex > 0) {
      try {
        ...
        setFile(null);
        getSuffixRotator()
            .rotate(getErrorManager(), file.toPath(), getNextSuffix(), maxBackupIndex);
        // start with new file.
        setFile(file);
      } catch (IOException e) {
