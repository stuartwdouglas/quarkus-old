...
public class SizeRotatingFileHandler extends FileHandler {
   ...
   * @throws java.io.FileNotFoundException if the file could not be found on open
   */
  public SizeRotatingFileHandler(final File file, final boolean append)
   ...
   * @throws java.io.FileNotFoundException if the file could not be found on open
   */
  public SizeRotatingFileHandler(final String fileName, final boolean append)
      throws FileNotFoundException {
   ...
   */
  public SizeRotatingFileHandler(
      final File file, final boolean append, final long rotateSize, final int maxBackupIndex)
      throws FileNotFoundException {
    super(file, append);
    this.rotateSize = rotateSize;
    this.maxBackupIndex = maxBackupIndex;
  ...
  public void setFile(final File file) throws FileNotFoundException {
    synchronized (outputLock) {
      ...
      super.setFile(file);
      if (outputStream != null) outputStream.currentSize = file == null ? 0L : file.length();
    }
  }

  /**
   * Indicates whether or a not the handler should rotate the file before the first log record is
   * written.
   *
  ...
  /**
   * Set to a value of {@code true} if the file should be rotated before the a new file is set. The
   * rotation only happens if the file names are the same and the file has a {@link
   * java.io.File#length() length} greater than 0.
   *
  ...
  /**
   * Sets the suffix to be appended to the file name during the file rotation. The suffix does not
   * play a role in determining when the file should be rotated.
   *
   * <p>The suffix must be a string understood by the {@link java.text.SimpleDateFormat}. Optionally
   * the suffix can end with {@code .gz} or {@code .zip} which will compress the file on rotation.
   *
   * <p>
   *
   * <p>If the suffix ends with {@code .gz} or {@code .zip} the file will be compressed on rotation.
   *
   * <p><b>Note:</b> The {@link #setMaxBackupIndex(int) maxBackupIndex} only takes into account
   * files rotated with same suffix. For example if the suffix pattern is {@code .yyyy-MM-dd} and
   * the size rotation is reached only files with the same date suffix will be purged. A file the
   * day before or after will not be purged.
   *
