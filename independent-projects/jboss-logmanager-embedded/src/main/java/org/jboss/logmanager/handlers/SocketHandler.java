...
public class SocketHandler extends ExtHandler {
  ...
  /**
   * Creates a socket handler with an address of {@linkplain java.net.InetAddress#getLocalHost()
   * localhost} and port of {@linkplain #DEFAULT_PORT 4560}.
   *
   ...
   */
  public SocketHandler(final Protocol protocol, final String hostname, final int port)
   ...
   *
   * <p>Note that is resets the {@linkplain #setClientSocketFactory(ClientSocketFactory) client
   * socket factory}.
   *
  ...
  /**
   * Enables or disables blocking when attempting to reconnect the socket when using a {@linkplain
   * Protocol#TCP TCP} or {@linkplain Protocol#SSL_TCP SSL TCP} connections.
   *
  ...
  /**
   * Sets the protocol to use. If the value is {@code null} the protocol will be set to {@linkplain
   * Protocol#TCP TCP}.
   *
   * <p>Note that is resets the {@linkplain #setSocketFactory(SocketFactory) socket factory} if it
   * was previously set.
   *
  ...
  /**
   * Sets the socket factory to use for creating {@linkplain Protocol#TCP TCP} or {@linkplain
   * Protocol#SSL_TCP SSL} connections.
   *
   * <p>Note that if the {@linkplain #setProtocol(Protocol) protocol} is set the socket factory will
   * be set to {@code null} and reset. Setting a value here also resets the {@linkplain
   * #setClientSocketFactory(ClientSocketFactory) client socket factory}.
   *
  ...
  /**
   * Sets the client socket factory used to create sockets. If {@code null} the {@linkplain
   * #setAddress(InetAddress) address} and {@linkplain #setPort(int) port} are required to be set.
   *
    ...
    synchronized (outputLock) {
      ...
      if (socketFactory == null) {
        if (protocol == Protocol.SSL_TCP) {
          clientSocketFactory =
              ClientSocketFactory.of(SSLSocketFactory.getDefault(), address, port);
        } else {
