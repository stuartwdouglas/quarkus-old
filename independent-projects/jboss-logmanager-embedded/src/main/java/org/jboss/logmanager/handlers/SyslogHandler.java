 ...
 *
 * <p>This handler can write to syslog servers that accept the <a
 * href="http://tools.ietf.org/html/rfc3164">RFC3164</a> and <a
 * href="http://tools.ietf.org/html/rfc5424">RFC5424</a> formats. Writes can be done via TCP, SSL
 * over TCP or UDP protocols. You can also override the {@link
 * #setOutputStream(java.io.OutputStream) output stream} if a custom protocol is needed.
 *
 * <p>
 *
 * <pre>
...
public class SyslogHandler extends ExtHandler {
  ...
  /**
   * Severity as defined by RFC-5424 (<a
   * href="http://tools.ietf.org/html/rfc5424">http://tools.ietf.org/html/rfc5424</a>) and RFC-3164
   * (<a href="http://tools.ietf.org/html/rfc3164">http://tools.ietf.org/html/rfc3164</a>).
   */
  public static enum Severity {
    ...
    public static Severity fromLevel(final Level level) {
      ...
      } else if (levelValue >= org.jboss.logmanager.Level.WARN.intValue()
          || levelValue >= Level.WARNING.intValue()) {
        return Severity.WARNING;
      } else if (levelValue >= org.jboss.logmanager.Level.INFO.intValue()) {
        return Severity.INFORMATIONAL;
      } else if (levelValue >= org.jboss.logmanager.Level.TRACE.intValue()
          || levelValue >= Level.FINEST.intValue()) {
  ...
  /**
   * Facility as defined by RFC-5424 (<a
   * href="http://tools.ietf.org/html/rfc5424">http://tools.ietf.org/html/rfc5424</a>) and RFC-3164
   * (<a href="http://tools.ietf.org/html/rfc3164">http://tools.ietf.org/html/rfc3164</a>).
   */
  ...
  public static enum SyslogType {
    /**
     * Formats the message according the the RFC-5424 specification (<a
     * href="http://tools.ietf.org/html/rfc5424#section-6">http://tools.ietf.org/html/rfc5424#section-6</a>
     */
    RFC5424,

    /**
     * Formats the message according the the RFC-3164 specification (<a
     * href="http://tools.ietf.org/html/rfc3164#section-4.1">http://tools.ietf.org/html/rfc3164#section-4.1</a>
     */
    RFC3164,
  }
  ...
  /**
   * Creates a new syslog handler that sends the messages to the server represented by the {@code
   * serverHostname} parameter on the port represented by the {@code port} parameter.
   *
  ...
  /**
   * Creates a new syslog handler that sends the messages to the server represented by the {@code
   * serverAddress} parameter on the port represented by the {@code port} parameter.
   *
   ...
   * @param hostname the name of the host the messages are being sent from see {@link
   *     #setHostname(String)} for details on the hostname
   * @throws IOException if an error occurs creating the UDP socket
   */
  public SyslogHandler(
      final String serverHostname, final int port, final Facility facility, final String hostname)
      throws IOException {
  ...
  private void sendMessage(
      final byte[] header, final ByteStringBuilder message, final byte[] trailer)
      throws IOException {
  ...
  /**
   * Gets app name used when formatting the message in RFC5424 format. By default the app name is
   * &quot;java&quot;.
   *
  ...
  /**
   * Sets app name used when formatting the message in RFC5424 format. By default the app name is
   * &quot;java&quot;. If set to {@code null} the {@link ExtLogRecord#getProcessName()} will be
   * used.
   *
  ...
  /**
   * Indicates whether or not a {@link org.jboss.logmanager.handlers.SyslogHandler.Protocol#TCP TCP}
   * or {@link org.jboss.logmanager.handlers.SyslogHandler.Protocol#SSL_TCP SSL TCP} connection
   * should block when attempting to reconnect.
   *
  ...
  /**
   * Enables or disables blocking when attempting to reconnect a {@link
   * org.jboss.logmanager.handlers.SyslogHandler.Protocol#TCP TCP} or {@link
   * org.jboss.logmanager.handlers.SyslogHandler.Protocol#SSL_TCP SSL TCP} protocol.
   *
   * <p>If set to {@code true} the {@code publish} methods will block when attempting to reconnect.
   * This is only advisable to be set to {@code true} if using an asynchronous handler.
   *
   * @param blockOnReconnect {@code true} to block when reconnecting or {@code false} to reconnect
   *     asynchronously discarding any new messages coming in
   */
  ...
  /**
   * Checks whether or not characters below decimal 32, traditional US-ASCII control values expect
   * {@code DEL}, are being escaped or not.
   *
   * @return {@code false}
   * @deprecated escaping message values is not required per <a
   *     href="http://tools.ietf.org/html/rfc5424#section-6.2">RFC5424</a> and is no longer
   ...
   *
   * <p>Set to {@code true} to escape characters within the message string that are below decimal
   * 32. These values are tradition US-ASCII control values. The values will be replaced in a {@code
   * #xxx} format where {@code xxx} is the octal value of the character being replaced.
   *
  ...
  /**
   * Returns the maximum length, in bytes, of the message allowed to be sent. The length includes
  ...
  /**
   * Sets the maximum length, in bytes, of the message allowed to tbe sent. Note that the message
   * length includes the header and the message itself.
   *
  ...
  /**
   * Returns the delimiter being used for the message if {@link #setUseMessageDelimiter(boolean) use
   * message delimiter} is set to {@code true}.
   *
   ...
   *
   * @param useDelimiter {@code true} to append the message with a delimiter, otherwise {@code
   *     false}
   */
   ...
   *
   * <p>This should be the name of the host sending the log messages, Note that the name cannot
   * contain any whitespace.
   *
   * <p>The hostname should be the most specific available value first. The order of preference for
   * the contents of the hostname is as follows:
   *
   * <ol>
   *   <li>FQDN
   *   <li>Static IP address
   *   <li>hostname
   *   <li>Dynamic IP address
   *   <li>{@code null}
   * </ol>
   *
   ...
   *
   * <p>See <a
   * href="http://tools.ietf.org/html/rfc6587#section-3.4.1">http://tools.ietf.org/html/rfc6587</a>
   ...
   * for more details on framing types.
   *
   * @param useCountingFraming {@code true} if the message being sent should be prefixed with the
   *     size of the message
   * @throws SecurityException if a security manager exists and if the caller does not have {@code
   *     LoggingPermission(control)} or the handler is {@link #protect(Object) protected}
   */
   ...
   * @throws UnknownHostException if no IP address for the host could be found, or if a scope_id was
   ...
   *
   * <p>Setting the output stream closes any already established connections or open output streams
   * and will not open any new connections until the output stream is set to {@code null}. The
   * {@link #setProtocol(org.jboss.logmanager.handlers.SyslogHandler.Protocol) protocol}, {@link
   * #setServerAddress(java.net.InetAddress), server address}, {@link #setServerHostname(String)
   * server hostname} or {@link #setPort(int) port} have no effect when the output stream is set.
   *
  ...
  /**
   * Checks if the message should truncated if the total length exceeds the {@link #getMaxLength()
   * maximum length}.
   *
  ...
  /**
   * Set to {@code true} if the message should be truncated if the total length the {@link
   * #getMaxLength() maximum length}.
   *
   * <p>Set to {@code false} if the message should be split and sent as multiple messages. The
   * header will remain the same for each message sent. The wrapping is not a word based wrap and
   * could split words between log messages.
   *
  ...
  protected byte[] createRFC5424Header(final ExtLogRecord record) throws IOException {
    ...
    if (millis <= 0) {
      ...
      // The follow can be changed to use a formatter with Java 7 pattern is
  ...
  private static String checkPrintableAscii(final String name, final String value) {
    if (value != null && PRINTABLE_ASCII_PATTERN.matcher(value).find()) {
      ...
      throw new IllegalArgumentException(
          String.format(
              "%s '%s' is invalid. The %s must be printable ASCII characters with no spaces.",
              upper, value, name));
    }
    return value;
  }
