 ...
 *
 * <p>If an {@link java.io.IOException IOException} occurs during a {@link #write(byte[], int, int)}
 * and a {@link javax.net.SocketFactory socket factory} was defined the stream will attempt to
 * reconnect indefinitely. By default additional writes are discarded when reconnecting. If you set
 * the {@link #setBlockOnReconnect(boolean) block on reconnect} to {@code true}, then the reconnect
 * will indefinitely block until the TCP stream is reconnected.
 *
 * <p>You can optionally get a collection of the errors that occurred during a write or reconnect.
 *
...
public class TcpOutputStream extends OutputStream implements FlushableCloseable {
   ...
   *
   * <p>Uses the {@link javax.net.SocketFactory#getDefault() default socket factory} to create the
   * socket.
   *
   * @param address the address to connect to
   * @param port the port to connect to
   * @throws IOException no longer throws an exception. If an exception occurs while attempting to
   *     connect the socket a reconnect will be attempted on the next write.
   */
   ...
   * @param address the address to connect to
   * @param port the port to connect to
   * @param blockOnReconnect {@code true} to block when attempting to reconnect the socket or {@code
   *     false} to reconnect asynchronously
   * @throws IOException no longer throws an exception. If an exception occurs while attempting to
   *     connect the socket a reconnect will be attempted on the next write.
   */
  public TcpOutputStream(final InetAddress address, final int port, final boolean blockOnReconnect)
      throws IOException {
   ...
   *
   * <p><strong>Using this constructor does not allow for any kind of reconnect if the connection is
   * dropped.</strong>
   *
   ...
   * Creates a new TCP output stream.
   *
   * <p>Creates a {@link java.net.Socket socket} from the {@code socketFactory} argument.
   *
  ...
  protected TcpOutputStream(
      final SocketFactory socketFactory, final InetAddress address, final int port)
      throws IOException {
  ...
  protected TcpOutputStream(
      ...
      final int port,
      final boolean blockOnReconnect)
      throws IOException {
    this(ClientSocketFactory.of(socketFactory, address, port), blockOnReconnect);
  }

  /**
   * Creates a new TCP stream which uses the {@link ClientSocketFactory#createSocket()} to create
   * the socket.
   *
  ...
  /**
   * Indicates whether or not the output stream is set to block when attempting to reconnect a TCP
   * connection.
   *
   ...
   *
   * <p>If set to {@code true} the {@code write} methods will block when attempting to reconnect.
   ...
   *
   * <p>The stream is said to be disconnected when an {@link java.io.IOException} occurs during a
   * write. Otherwise a stream is considered connected.
   *
  ...
  /**
   * Attempts to reconnect the socket if required and allowed. Invocations of this method must be
   * locked by the {@link #outputLock}.
   */
    ...
    public void run() {
      ...
      while (socketFactory != null && !connected) {
        ...
        try {
          ...
          // It's possible the thread was interrupted, if we're not connected we should clean up the
          // socket
          if (!connected) {
            safeClose(socket);
          }
