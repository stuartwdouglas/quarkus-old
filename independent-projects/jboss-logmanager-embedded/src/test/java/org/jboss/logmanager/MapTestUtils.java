...
public class MapTestUtils {
  ...
  public static <K, V> void compareMaps(final Map<K, V> m1, final Map<K, V> m2) {
    ...
    for (K key : m1.keySet()) {
      ...
      Assert.assertEquals(
          String.format(
              "Value %s from the first map does not match value %s from the second map with key %s.",
              value1, value2, key),
          value1,
          value2);
    }
  ...
  /**
   * A helper to easily build maps. The resulting map is immutable and the order is predictable with
   * the {@link #add(Object, Object)} order.
   *
   * @author <a href="mailto:jperkins@redhat.com">James R. Perkins</a>
   */
