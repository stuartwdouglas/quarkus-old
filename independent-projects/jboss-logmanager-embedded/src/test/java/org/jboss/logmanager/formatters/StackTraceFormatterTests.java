...
public class StackTraceFormatterTests {
  ...
  public void compareCauseStackTrace() {
    final RuntimeException e =
        new RuntimeException("Test Exception", new IllegalStateException("Cause"));
  ...
  private void checkMessage(
      final String msg, final String text, final boolean shouldExist, final int depth) {
    final boolean test = (shouldExist || depth < 0);
    Assert.assertEquals(
        String.format(
            "Depth %d should %s contained \"%s\": %s",
            depth, (test ? "have" : "not have"), text, msg),
        msg.contains(text),
        test);
  }
