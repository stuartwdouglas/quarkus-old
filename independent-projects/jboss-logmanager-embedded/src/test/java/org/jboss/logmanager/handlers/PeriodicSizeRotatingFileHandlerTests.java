...
public class PeriodicSizeRotatingFileHandlerTests extends AbstractHandlerTest {
  private static final String FILENAME = "rotating-file-handler.log";
  ...
  private void testArchiveRotate(final String dateSuffix, final String archiveSuffix)
      throws Exception {
    final String currentDate =
        dateSuffix == null ? "" : LocalDate.now().format(DateTimeFormatter.ofPattern(dateSuffix));
    PeriodicSizeRotatingFileHandler handler = new PeriodicSizeRotatingFileHandler();
    configureHandlerDefaults(handler);
    handler.setRotateSize(1024L);
    handler.setMaxBackupIndex(2);
  ...
  private void testPeriodicAndSizeRotate0(int handlerPeriod, int logMessagePeriod, boolean testSize)
      throws Exception {
    int logCount = 1;
    if (testSize) {
      logCount = 100;
