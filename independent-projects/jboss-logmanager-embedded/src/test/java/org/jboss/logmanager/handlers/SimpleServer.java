...
abstract class SimpleServer implements Runnable, AutoCloseable {
  ...
  static SimpleServer createTcpServer(final int port) throws IOException {
    final SimpleServer server =
        new TcpServer(ServerSocketFactory.getDefault(), new LinkedBlockingDeque<>(), port);
    server.start();
    return server;
  }
  ...
  static SimpleServer createTlsServer(final int port) throws IOException, GeneralSecurityException {
    final SimpleServer server =
        new TcpServer(SSLServerSocketFactory.getDefault(), new LinkedBlockingDeque<>(), port);
    server.start();
    return server;
  }
  ...
  private static class TcpServer extends SimpleServer {
    ...
    private TcpServer(
        ...
        final BlockingQueue<String> data,
        final int port)
        throws IOException {
      super(data);
