...
public class SyslogHandlerTests {
  ...
  public void testRFC5424Tcp() throws Exception {
    ...
    String expectedMessage =
        "<14>1 2012-01-09T04:39:22.000"
            + calculateTimeZone(cal)
            + " test java "
            + handler.getPid()
            + " - - "
            + BOM
  ...
  private void testMultibyteTruncation(
      final String part1, final String part2, final int charsToTruncate) throws Exception {
    // Setup the handler
    handler.setSyslogType(SyslogType.RFC5424);
    handler.setUseMessageDelimiter(false);
    final ByteArrayOutputStream out = new ByteArrayOutputStream();
    ...
    // the multi-byte character was not written at the end
    out.reset();
    handler.setTruncate(true);
  ...
  private static String createString(final ByteArrayOutputStream out)
