...
import org.jboss.shamrock.deployment.cdi.CdiResourceBuildItem;
...
public final class HibernateResourceProcessor {
    ...
    public BeanConfiguratorBuildItem build(BytecodeRecorder recorder, JPADeploymentTemplate template, List<PersistenceUnitDescriptorBuildItem> descItems, CombinedIndexBuildItem index) throws Exception {
        ...
        return new BeanConfiguratorBuildItem(template.initMetadata(descriptors, scanner));
    }
    ...
    @BuildStep
    @Record(staticInit = true)
    public CdiResourceBuildItem build(JPADeploymentTemplate template,
                                      Capabilities capabilities, BuildProducer<BeanConfiguratorBuildItem> buildProducer,
                                      List<PersistenceUnitDescriptorBuildItem> descriptors) throws Exception {

        buildProducer.produce(new BeanConfiguratorBuildItem(template.initializeJpa(capabilities.isCapabilityPresent(Capabilities.TRANSACTIONS))));
        // Bootstrap all persistence units
        for (PersistenceUnitDescriptorBuildItem persistenceUnitDescriptor : descriptors) {
            buildProducer.produce(new BeanConfiguratorBuildItem(template.registerPersistenceUnit(persistenceUnitDescriptor.getDescriptor().getName())));
        }
        buildProducer.produce(new BeanConfiguratorBuildItem(template.initDefaultPersistenceUnit()));
        return new CdiResourceBuildItem();
    }
