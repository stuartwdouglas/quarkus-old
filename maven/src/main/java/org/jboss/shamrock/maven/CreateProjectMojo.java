...
/**
 * This goal helps in setting up Shamrock Maven project with shamrock-maven-plugin, with sensible
 * defaults
 */
...
public class CreateProjectMojo extends AbstractMojo {
  ...
  public static final String PLUGIN_KEY =
      MavenConstants.PLUGIN_GROUPID + ":" + MavenConstants.PLUGIN_ARTIFACTID;
  ...
  @Component private SetupTemplates templates;

  @Component private Extensions ext;
  ...
  private void printUserInstructions(File pomFile) {
    ...
    getLog()
        .info(
            ansi()
                .a("Your new application has been created in ")
                .bold()
                .a(pomFile.getAbsolutePath())
                .boldOff()
                .toString());
    getLog()
        .info(
            ansi()
                .a("Navigate into this directory and launch your application with ")
                .bold()
                .fg(Ansi.Color.CYAN)
    ...
    getLog()
        .info(
            ansi()
                .a("Your application will be accessible on ")
                .bold()
                .fg(Ansi.Color.CYAN)
  ...
  private void addNativeProfile(Model model) {
    ...
    Plugin plg =
        plugin(
            MavenConstants.PLUGIN_GROUPID,
            MavenConstants.PLUGIN_ARTIFACTID,
  ...
  private File createPomFileFromUserInputs() throws MojoExecutionException {
    ...
    try {
      ...
      if (projectGroupId == null) {
        projectGroupId =
            prompter.promptWithDefaultValue("Set the project groupId", "io.jboss.shamrock.sample");
      }

      // If the user does not specify the artifactId, we switch to the interactive mode.
      if (projectArtifactId == null) {
        projectArtifactId =
            prompter.promptWithDefaultValue("Set the project artifactId", "my-shamrock-project");
        ...
        if (shamrockVersion == null) {
          shamrockVersion =
              prompter.promptWithDefaultValue(
                  "Set the Shamrock version", MojoUtils.get(VERSION_PROP));
        }
        ...
        if (className == null) {
          className =
              prompter.promptWithDefaultValue(
                  ...
                  projectGroupId.replace("-", ".").replace("_", ".") + ".HelloResource");
      ...
      context.put(
          "shamrockVersion",
          shamrockVersion != null ? shamrockVersion : MojoUtils.get(VERSION_PROP));
