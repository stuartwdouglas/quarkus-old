...
public class DevMojo extends AbstractMojo {
  ...
  /**
   * If this server should be started in debug mode. The default is to start in debug mode without
   * suspending and listen on port 5005. It supports the following options:
   *
   * <table>
   * <tr>
  ...
  /**
   * This value is intended to be set to true when some generated bytecode is erroneous causing the
   * JVM to crash when the verify:none option is set (which is on by default)
   */
  ...
  public void execute() throws MojoFailureException {
    ...
    if (!found) {
      getLog()
          .warn(
              "The shamrock-maven-plugin build goal was not configured for this project, skipping shamrock:dev as this is assumed to be a support library. If you want to run shamrock dev on this project make sure the shamrock-maven-plugin is configured with a build goal.");
      return;
    }
    ...
    if (!buildDir.isDirectory() || !new File(buildDir, "classes").isDirectory()) {
      throw new MojoFailureException(
          "The project has no output yet, run `mvn compile shamrock:dev`.");
    }
    ...
    try {
      ...
      if (debug == null) {
        ...
        try (Socket socket =
            new Socket(InetAddress.getByAddress(new byte[] {127, 0, 0, 1}), 5005)) {
      ...
      classPath
          .append(wiringClassesDirectory.toPath().toAbsolutePath().toUri().toURL().toString())
          .append("/");
      classPath.append(' ');
      ...
      if (fakereplace) {
        ...
        if (!target.exists()) {
          ...
          URL resource =
              getClass().getClassLoader().getResource("org/fakereplace/core/Fakereplace.class");
          if (resource == null) {
            throw new RuntimeException("Could not determine Fakereplace location");
          }
          String filePath = resource.getPath();
          try (FileInputStream in =
              new FileInputStream(filePath.substring(5, filePath.lastIndexOf('!')))) {
      ...
      URL classFile =
          DevModeMain.class
              .getClassLoader()
              .getResource(DevModeMain.class.getName().replace('.', File.separatorChar) + ".class");
      Path path;
