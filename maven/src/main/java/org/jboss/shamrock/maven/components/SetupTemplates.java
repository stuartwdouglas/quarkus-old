...
public class SetupTemplates {
  ...
  public void createNewProjectPomFile(Map<String, String> context, File pomFile)
  ...
  public void generate(MavenProject project, String path, String className, Log log)
      throws MojoExecutionException {
  ...
  public void createIndexPage(Map<String, String> context, File basedir, Log log)
      throws MojoExecutionException {
    // Generate index page
    ...
    if (!index.exists()) {
      try (Writer out = new FileWriter(index)) {
        ...
        temp.process(context, out);
        log.info(
            "Welcome page created in src/main/resources/META-INF/resources/" + index.getName());
      } catch (Exception e) {
  ...
  public void createDockerFile(Map<String, String> context, File basedir, Log log)
      throws MojoExecutionException {
  ...
  public void createConfiguration(File basedir, Log log) throws MojoExecutionException {
    ...
    if (!file.exists()) {
      try {
        Files.write(
            file.toPath(),
            Arrays.asList("# Configuration file", "key = value"),
