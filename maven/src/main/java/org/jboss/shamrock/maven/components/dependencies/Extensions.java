...
public class Extensions {
  ...
  private List<Dependency> getDependenciesFromBom(
      MavenSession session, List<ArtifactRepository> repositories) throws MojoExecutionException {
    String bomCoordinates =
        PLUGIN_GROUPID
            + ":"
            + MojoUtils.get("bom-artifactId")
            + ":"
          ...
          exts.stream()
              .filter(
                  d -> {
                    ...
                    boolean machName =
                        d.getName().toLowerCase().contains(dependency.trim().toLowerCase());
                    return hasTag || machName;
                  })
              .findAny();

      if (optional.isPresent()) {
        if (!MojoUtils.hasDependency(
            model, optional.get().getGroupId(), optional.get().getArtifactId())) {
  ...
  private boolean containsBOM(Model model) {
    ...
    return dependencies
        ...
        .anyMatch(
            dependency ->
                dependency.getArtifactId().equalsIgnoreCase(MojoUtils.get("bom-artifactId")));
  }

  /**
   * Retrieves the Maven Project associated with the given artifact String, in the form of <code>
   * groupId:artifactId[:version]</code>. This resolves the POM artifact at those coordinates and
   * then builds the Maven project from it.
   *
  ...
  protected MavenProject getMavenProject(
      String artifactString, MavenSession session, List<ArtifactRepository> repositories)
      throws MojoExecutionException {
    ...
    try {
      ...
      throw new MojoExecutionException(
          "Unable to get the POM for the artifact '"
              + artifactString
  ...
  protected ArtifactCoordinate getArtifactCoordinate(
