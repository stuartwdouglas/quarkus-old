...
public class CreateProjectMojoIT extends MojoTestBase {
  ...
  public void testProjectGenerationFromScratch() throws MavenInvocationException, IOException {
    ...
    assertThat(new File(testDir, "src/main/resources/META-INF/microprofile-config.properties"))
        .isFile();
    assertThat(new File(testDir, "src/main/resources/META-INF/resources/index.html")).isFile();

    String config =
        Files.asCharSource(
                new File(testDir, "src/main/resources/META-INF/microprofile-config.properties"),
                Charsets.UTF_8)
            .read();
    ...
    String index =
        Files.asCharSource(
                new File(testDir, "src/main/resources/META-INF/resources/index.html"),
                Charsets.UTF_8)
            .read();
  ...
  public void testProjectGenerationFromMinimalPom() throws Exception {
    ...
    assertThat(FileUtils.readFileToString(new File(testDir, "pom.xml"), "UTF-8"))
        .contains(
            ...
            CreateProjectMojo.PLUGIN_VERSION_PROPERTY,
            MavenConstants.PLUGIN_GROUPID);
    assertThat(new File(testDir, "src/main/java")).isDirectory();

    assertThat(new File(testDir, "src/main/resources/META-INF/microprofile-config.properties"))
        .doesNotExist();
    assertThat(new File(testDir, "src/main/resources/META-INF/resources/index.html"))
        .doesNotExist();

    assertThat(FileUtils.readFileToString(new File(testDir, "pom.xml"), "UTF-8"))
        .containsIgnoringCase(MojoUtils.get("bom-artifactId"));
  ...
  public void testProjectGenerationFromMinimalPomWithResource() throws Exception {
    testDir =
        initProject(
            "projects/simple-pom-it", "projects/project-generation-from-empty-pom-with-resource");
    assertThat(testDir).isDirectory();
    init(testDir);
    Properties properties = new Properties();
    properties.put("projectGroupId", "org.acme");
  ...
  public void testProjectGenerationFromScratchWithExtensions() throws Exception {
    ...
    assertThat(new File(testDir, "src/main/java/org/acme/ShamrockApplication.java")).doesNotExist();
    assertThat(FileUtils.readFileToString(new File(testDir, "pom.xml"), "UTF-8"))
        .contains("shamrock-jaxrs-deployment", "shamrock-metrics-deployment")
    ...
    System.out.println(
        model
            .getDependencies()
            .stream()
            .map(d -> d.getManagementKey() + "[" + d.getVersion() + "]")
                ...
                .anyMatch(
                    d ->
                        d.getArtifactId().equalsIgnoreCase("commons-io")
                            && d.getVersion().equalsIgnoreCase("2.5")))
        .isTrue();
  }
  ...
  private void setup(Properties params) throws MavenInvocationException, FileNotFoundException {
    ...
    PrintStreamLogger logger =
        new PrintStreamLogger(new PrintStream(new FileOutputStream(log)), InvokerLogger.DEBUG);
