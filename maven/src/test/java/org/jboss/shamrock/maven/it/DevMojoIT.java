...
public class DevMojoIT extends MojoTestBase {
  ...
  public void testThatTheApplicationIsReloadedOnJavaChange()
    ...
    await()
        ...
        .until(() -> getHttpResponse("/hello").contains(uuid));
    sleep();
    ...
    await()
        .pollDelay(100, TimeUnit.MILLISECONDS)
        .atMost(1, TimeUnit.MINUTES)
        .until(() -> getHttpResponse("/hello").contains("carambar"));
  }

  @Test
  public void testThatTheApplicationIsReloadedOnNewResource()
    ...
    await()
        .pollDelay(100, TimeUnit.MILLISECONDS)
        .atMost(1, TimeUnit.MINUTES)
        .until(() -> getHttpResponse("/foo").contains("bar"));
  }

  @Test
  @Ignore("Issue https://github.com/protean-project/shamrock/issues/245")
  public void testThatTheApplicationIsReloadedOnNewServlet()
      throws MavenInvocationException, IOException {
    ...
    await()
        .pollDelay(100, TimeUnit.MILLISECONDS)
        .atMost(1, TimeUnit.MINUTES)
        .until(() -> getHttpResponse("/my").contains("hello from servlet"));
  ...
  @Test
  public void testThatTheApplicationIsReloadedOnConfigChange()
      throws MavenInvocationException, IOException {
  ...
  @Test
  public void testThatApplicationRecoversCompilationIssue()
      throws MavenInvocationException, IOException {
    ...
    String uuid = UUID.randomUUID().toString();
    filter(
        source, ImmutableMap.of("return \"hello\";", "return \"" + uuid + "\"")); // No semi-colon
  ...
  public void testThatNewBeanAreDiscovered() throws IOException, MavenInvocationException {
    ...
    filter(
        resource,
        Collections.singletonMap("String greeting;", "String greeting;\n @Inject MyBean bean;"));
    ...
    await()
        .pollDelay(100, TimeUnit.MILLISECONDS)
        .atMost(1, TimeUnit.MINUTES)
        .until(() -> getHttpResponse("/hello").contains("message"));
    ...
    await()
        .pollDelay(100, TimeUnit.MILLISECONDS)
        .atMost(1, TimeUnit.MINUTES)
        .until(() -> getHttpResponse("/hello").contains("foobarbaz"));
  }
  ...
  public void testErrorMessageWhenNoJavaSources() throws IOException, MavenInvocationException {
    ...
    running = new RunningInvoker(testDir, false);
    MavenProcessInvocationResult result =
        running.execute(Arrays.asList("compile", "shamrock:dev"), Collections.emptyMap());
  ...
  public void testErrorMessageWhenNoTarget() throws IOException, MavenInvocationException {
    ...
    running = new RunningInvoker(testDir, false);
    MavenProcessInvocationResult result =
        running.execute(Collections.singletonList("shamrock:dev"), Collections.emptyMap());
    await().until(() -> result.getProcess() != null && !result.getProcess().isAlive());
    assertThat(running.log()).containsIgnoringCase("BUILD FAILURE");
  }
