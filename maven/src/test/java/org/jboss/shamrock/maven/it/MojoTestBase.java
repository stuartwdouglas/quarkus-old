...
public class MojoTestBase {
  ...
  public static void init() {
    ...
    VARIABLES =
        ImmutableMap.of(
            ...
            MojoUtils.get("restAssuredVersion"));
  }
  ...
  public static File initProject(String name, String output) {
    ...
    try {
      System.out.println(
          "Copying " + in.getAbsolutePath() + " to " + out.getParentFile().getAbsolutePath());
  ...
  public static void installPluginToLocalRepository(File local) {
    File repo =
        new File(
            local,
            MavenConstants.PLUGIN_GROUPID.replace(".", "/")
                + "/"
                + MavenConstants.PLUGIN_ARTIFACTID
                + "/"
                + MojoTestBase.VERSION);
    if (!repo.isDirectory()) {
    ...
    File plugin =
        new File("target", MavenConstants.PLUGIN_ARTIFACTID + "-" + MojoTestBase.VERSION + ".jar");
    if (!plugin.isFile()) {
      File[] files =
          new File("target")
              .listFiles(
                  file ->
                      file.getName().startsWith(MavenConstants.PLUGIN_ARTIFACTID)
    ...
    try {
      ...
      String installedPomName =
          MavenConstants.PLUGIN_ARTIFACTID + "-" + MojoTestBase.VERSION + ".pom";
      FileUtils.copyFile(new File("pom.xml"), new File(repo, installedPomName));
    } catch (IOException e) {
      throw new RuntimeException(
          "Cannot copy the plugin jar, or the pom file, to the local repository", e);
    }
  }
  ...
  static void installJarToLocalRepository(String local, String name, File jar) {
    ...
    try {
      ...
      FileUtils.write(
          new File(repo, installedPomName),
          "<project>\n"
      ...
      throw new RuntimeException(
          "Cannot copy the jar, or the pom file, to the local repository", e);
    }
  }
    ...
    await()
        ...
        .until(
            () -> {
              try {
                URL url =
                    new URL("http://localhost:8080" + ((path.startsWith("/") ? path : "/" + path)));
