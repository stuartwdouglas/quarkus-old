...
/**
 * An implementation of {@link DefaultInvoker} launching Maven, but does not wait for the
 * termination of the process. The launched process is passed in the {@link InvocationResult}.
 *
...
public class MavenProcessInvoker extends DefaultInvoker {
  ...
  public InvocationResult execute(InvocationRequest request) throws MavenInvocationException {
    ...
    try {
      ...
      throw new MavenInvocationException(
          "Error configuring command-line. Reason: " + e.getMessage(), e);
    }
  ...
  private static Process executeCommandLine(
      Commandline cl, StreamConsumer systemOut, StreamConsumer systemErr)
      throws CommandLineException {
    if (cl == null) {
