...
abstract class AbstractShamrockRunListener extends RunListener {
  ...
  private Set<ShamrockTestResourceLifecycleManager> getTestResources(Class<?> testClass) {
    ...
    Set<Class<? extends ShamrockTestResourceLifecycleManager>> testResourceRunnerClasses =
        new LinkedHashSet<>();

    for (AnnotationInstance annotation :
        index.getAnnotations(DotName.createSimple(ShamrockTestResource.class.getName()))) {
      try {
        testResourceRunnerClasses.add(
            (Class<? extends ShamrockTestResourceLifecycleManager>)
                Class.forName(annotation.value().asString()));
    ...
    for (Class<? extends ShamrockTestResourceLifecycleManager> testResourceRunnerClass :
        testResourceRunnerClasses) {
      try {
        testResourceRunners.add(testResourceRunnerClass.getConstructor().newInstance());
          ...
          | SecurityException e) {
        throw new RuntimeException(
            "Unable to instantiate the test resource " + testResourceRunnerClass);
      }
    }
  ...
  private IndexView indexTestClasses(Class<?> testClass) {
    ...
    try {
      Files.walkFileTree(
          getTestClassesLocation(testClass),
