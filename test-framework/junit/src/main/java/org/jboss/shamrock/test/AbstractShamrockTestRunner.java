...
abstract class AbstractShamrockTestRunner extends BlockJUnit4ClassRunner {
  ...
  private final BiFunction<Class<?>, RunNotifier, AbstractShamrockRunListener>
      shamrockRunListenerSupplier;

  public AbstractShamrockTestRunner(
      Class<?> klass,
  ...
  public void run(final RunNotifier notifier) {
    if (first) {
      first = false;
      shamrockRunListener =
          shamrockRunListenerSupplier.apply(getTestClass().getJavaClass(), notifier);
