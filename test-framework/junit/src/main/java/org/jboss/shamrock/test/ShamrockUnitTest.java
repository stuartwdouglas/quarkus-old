...
public class ShamrockUnitTest extends BlockJUnit4ClassRunner {
  ...
  protected Object createTest() throws Exception {
    InjectionInstance<?> factory =
        InjectionFactoryTemplate.currentFactory().create(getTestClass().getJavaClass());
  ...
  private static Class<?> doSetup(Class<?> testClass) {
    try {
      ...
      try {
        ...
        if (shouldFail != null) {
          if (e instanceof RuntimeException) {
            ...
            if (cause != null) {
              assertTrue(
                  cause.getClass() + " is not assignable to the expected: " + shouldFail.value(),
                  shouldFail.value().isAssignableFrom(cause.getClass()));
            }
          } else {
      ...
      return Class.forName(
          testClass.getName(), true, Thread.currentThread().getContextClassLoader());
  ...
  private static void exportArchive(
      Path deploymentDir, Class<?> testClass, Method deploymentMethod) {
    try {
