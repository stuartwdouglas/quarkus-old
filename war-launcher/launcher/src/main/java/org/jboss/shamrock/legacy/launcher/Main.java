...
/**
 * For some reason when using class-path entries something seems broken in java service loaders, as
 * getResources() will only every return a single resource from the app class loader.
 *
 * <p>To get around this the legacy integration is broken up into a launcher and the runner, with
 * this launcher just setting up class loading.
 *
 * <p>This has the advatange than anything added to he lib dir will automatically be added to the
 * class path.
 */
public class Main {
  ...
  public static void main(String... args) {
    ...
    if (location.getProtocol().equals("jar")) {
      ...
      String loc =
          location.getPath().substring(0, location.getPath().length() - classFileName.length());
      File file = new File(loc);
      libDir = new File(file.getParentFile(), "lib");
    } else {
